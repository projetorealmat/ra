...
$\sabs{x-c} < \delta$, and $x \neq c$.  As $\delta > 0$ and $c$ is a cluster
...
= \epsilon.
\end{equation*}
As $\sabs{L_1-L_2} < \epsilon$ for arbitrary $\epsilon > 0$, we have
$L_1 = L_2$.
\end{proof}
...
\end{equation*}
Take $x \neq c$ such that $\sabs{x-c} < \delta$.  In particular,
$\sabs{x-c} < 1$.  By the reverse triangle inequality,
\begin{equation*}
...
A graph of a function that is 1 when the independent
variable x is 0, then when the independent variable
x is any bigger than 0, the function is equal to x.
It is indicated that the function is not defined at
x equal to 1 or any x greater, nor at any x less than 0.}
...
For $x \in [0,1)$, $x \neq 0$, and $\sabs{x-0} < \delta$, we get
\begin{equation*}
...
The graphs of functions on an interval around the origin.
On the left the function oscillates
between minus 1 and 1
faster and faster as x approaches 0, so much so that near the origin
the graph appears as solid black.
On the right, the function again oscillates faster and faster
as x approaches 0, but now the amplitude also goes down to 0
and so what sort of seems like a solid black region is really like the
tip of a triangle.}
...
such that $x_n \neq 0$ for all $n$, and such that $\lim_{n\to\infty} x_n = 0$.
...
\sabs{x_n} .
\end{equation*}
As $x_n$ goes to zero, $\sabs{x_n}$ goes to zero, and hence
...
\begin{proof}
Take $\{ x_n \}_{n=1}^\infty$ to be a sequence of numbers in $S \setminus \{ c \}$
...
\to \R$ be a function.
Suppose
$A \subset S$ is such that for some $\alpha > 0$, we have
...
Since $A \subset S$, if $( A \setminus \{ c\} ) \cap
...
the hypothesis in the proposition, is the
...
Let $f \colon S \to \R$ be a function and let $c \in \R$.  If
...
\begin{exercise}
Find the limit (and prove it of course) or prove that the limit does not
exist:
...
\begin{exercise}
Suppose $f \colon \R \to \R$ is a function such that for every 
...
(Bolzano, Cauchy, and finally Weierstrass) to formulate correctly and its final
...
A diagram of the graph of a function y equals f of x,
near a point x equals c.  The graph is shown and the
strip within delta of c is shown and a strip within
epsilon of f of c is shown.  The box that fits within
both strips is shaded, and within this range the graph
fits within the vertical range.}
...
Note that $\delta$ depends not only
on $\epsilon$, but also on $c$;  we need not pick
...
\propref{prop:contalg}, give a quick proof of a much more general result.
...
\item The function $h \colon S \to \R$ defined by
$h(x) \coloneqq f(x)g(x)$ is continuous at $c$.
\item If $g(x) \neq 0$ for all $x \in S$, the function $h \colon S \to \R$
...
$\lim_{n\to\infty} x_n = c$.
As $g$ is continuous at $c$, we have  that$\bigl\{ g(x_n) \bigr\}_{n=1}^\infty$ converges to $g(c)$.
As $f$ is continuous at $g(c)$, we have that $\bigl\{ f\bigl(g(x_n)\bigr)
...
$x^2$ is continuous on the interval $[-1,1]$ (the range of $\sin$).
...
from the third item of \propref{contbasic:prop}.
...
A graph of a function that is minus 1 for negative x
and 1 for nonnegative x.  The points minus 1,
minus a half, minus a third, etc. are marked on
the x-axis and the corresponding values on
the graph are marked with dots.}
...
\begin{example}
For an extreme example, consider the
...
of irrational numbers in $(0,1)$ such that $\lim_{n\to\infty} x_n = c$ (why can we?).  Then $f(x_n) = 0$
and so $\lim_{n\to\infty} f(x_n) = 0$, but $f(c) = 1$.
If $c$ is irrational, take a sequence
$\{ x_n \}_{n=1}^\infty$
of rational numbers in $(0,1)$
...
A graph of a function that is composed of dots.
These dots seem arranged in what seems like
various triangles, but it is clear this
function is discontinuous at all those points,
and there are more and more of such points
the closer we get to the x-axis.}
...
the line drawn between $\bigl(a,f(a)\bigr)$ and $\bigl(b,f(b)\bigr)$ 
...
f(x) = a_d x^d + a_{d-1} x^{d-1} + \cdots + a_1 x + a_0 ,
\end{equation*}
where $a_d \neq 0$.  We divide by $a_d$ to obtain a 
...
With Bolzano's
theorem, we can prove the existence of a $k$th root of any positive number
...
The proof is left as \exerciseref{exercise:meanvaluepropsin1x}.
\end{example}
...
A graph of a function y equals f of x on the interval
from a to b marked in a bold line.  On the vertical
axis, the set of all values achieved by the function
is marked as f of the closed interval from a to b.}
