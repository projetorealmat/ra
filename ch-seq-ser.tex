...
The convergence and the limit of a sequence only depend on its tail.
...
%mbxlatex \myincludepdft{tail_of_sequence_cd}{A diagram with
%mbxlatex three implication arrows.  First arrow from (i) to (ii)
%mbxlatex is labeled with "to prove".  Second arrow from (ii) to (iii)
%mbxlatex is not labeled.  Third arrow from (iii) to (i) is
%mbxlatex labeled with "to prove".}
%mbxlatex \end{center}
...
\end{equation*}
where $a \neq b$.  Prove that $\{ x_n \}_{n=1}^\infty$ is not convergent, without
