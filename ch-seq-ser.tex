...
A análise trata essencialmente de limites. O tipo mais básico de limite
é o limite de uma sequência de números reais.
Já vimos sequências de maneira informal. Vamos dar a
definição formal.

\begin{defn}
Uma \emph{\myindex{sequência}} (de números reais) é uma função $x \colon \N \to
\R$. Em vez de $x(n)$,
costumamos denotar o $n$-ésimo elemento da sequência por $x_n$.
Para denotar uma sequência, escrevemos%
\footnote{É comum usar $\{ x_n \}$ ou $\{ x_n \}_n$ como abreviação.}%
\glsadd{not:sequence}%
\begin{equation*}
\{ x_n \}_{n=1}^\infty.
\end{equation*}

Uma sequência $\{ x_n \}_{n=1}^\infty$ é \emph{limitada}\index{sequência limitada} se
a função correspondente é limitada. Isto é, se
existe um $B \in \R$ tal que
\begin{equation*}
\sabs{x_n} \leq B \qquad \text{para todo } n \in \N.
\end{equation*}
Em outras palavras, a sequência $\{x_n\}_{n=1}^\infty$ é limitada sempre que
o conjunto $\{ x_n : n \in \N \}$
é limitado.
Definimos de maneira semelhante os termos
\emph{limitada inferiormente}\index{limitada inferiormente!sequência} e
\emph{limitada superiormente}\index{limitada superiormente!sequência}.
\end{defn}

Quando precisamos
dar uma sequência concreta, muitas vezes apresentamos cada termo por meio de uma fórmula
em função de $n$.
Por exemplo, $\{ \nicefrac{1}{n} \}_{n=1}^\infty$ representa
a sequência $1, \nicefrac{1}{2}, \nicefrac{1}{3}, \nicefrac{1}{4},
\nicefrac{1}{5}, \ldots$.
A sequência $\{ \nicefrac{1}{n} \}_{n=1}^\infty$
é limitada (basta tomar $B=1$). Por outro lado, a sequência
$\{ n \}_{n=1}^\infty$ representa
$1,2,3,4,\ldots$, e essa sequência não é limitada (por quê?).

Embora a notação de uma sequência
seja parecida\footnote{\cite{BS} usam $(x_n)_{n=1}^\infty$ para denotar
uma sequência, em vez de $\{ x_n \}_{n=1}^\infty$, que é a notação usada em \cite{Rudin:baby}.
As duas são comuns.}
com a de um conjunto, as noções são
distintas. Por exemplo, a sequência $\bigl\{ {(-1)}^n \bigr\}_{n=1}^\infty$ é a sequência
$-1,1,-1,1,-1,1,\ldots$, enquanto o conjunto de valores, chamado de
\emph{imagem da sequência}\index{imagem de uma sequência},
é simplesmente o conjunto $\{ -1, 1 \}$. Escrevemos esse conjunto
como $\bigl\{ {(-1)}^n : n \in \N \bigr\}$.
% When ambiguity can arise, we
%use the words \emph{sequence} or \emph{set} to distinguish the two
%concepts.

Outro exemplo de sequência é a \emph{\myindex{sequência constante}}.
Trata-se de uma sequência $\{ c \}_{n=1}^\infty = c,c,c,c,\ldots$ que consiste
em um único valor constante $c \in \R$ repetido indefinidamente.
...
\begin{defn}
Dizemos que uma sequência $\{ x_n \}_{n=1}^\infty$ \emph{converge} para um número
$x \in \R$ se, para todo $\epsilon > 0$, existe um $M \in \N$ tal
que $\sabs{x_n - x} < \epsilon$ para todo $n \geq M$.
O número $x$ é chamado de \emph{limite} da sequência.
Demonstraremos que o limite $x$ é único; por isso, escrevemos%
\footnote{No texto, isso pode ser representado como $\lim_{n\to\infty} x_n$.}
\glsadd{not:limseq}%
\begin{equation*}
...
Uma sequência
que converge é chamada de \emph{convergente}\index{convergente!sequência}.
Caso contrário, dizemos que a sequência \emph{diverge}
ou que é
\emph{divergente}\index{divergente!sequência}.
\end{defn}

Em breve, na \propref{prop:limisunique}, mostraremos que o limite $x$ é sempre
único, quando existe. Faz sentido falar do limite de
uma sequência; basta mostrar que ela converge para um único
número.
Nos próximos exemplos, vamos fingir
que já demonstramos a unicidade dos limites.

Intuitivamente, dizer que o limite é $x$ significa que, a partir de algum ponto,
todos os termos da sequência ficam próximos de $x$. Mais precisamente,
podemos chegar arbitrariamente perto do limite, desde que avancemos o suficiente
na sequência. Isso não significa que algum dia atingiremos o limite. É possível,
e bastante comum, que nenhum $x_n$ da sequência seja igual ao
limite $x$.
Ilustramos o conceito na \figureref{figsequenceconvergence}. Na
figura, primeiro pensamos na sequência como um gráfico, pois ela é uma função de
$\N$. Em seguida, também a representamos como uma sequência de pontos marcados na reta
real.

\begin{myfigureht}
\myincludepdft{sequence-convergence_full}{%
Dois diagramas.
O primeiro é o gráfico de uma sequência: os valores estão no eixo vertical e
o índice está no eixo horizontal.
No eixo vertical,
o número x está marcado e há uma linha horizontal.
Uma faixa entre
x menos épsilon e x mais épsilon está sombreada. Os valores
da sequência ficam, em sua maioria, fora da faixa sombreada
até o índice 7, marcado como M. Todos os valores
seguintes estão dentro da faixa.
No segundo diagrama, o eixo x é horizontal, com
o número x marcado e os valores x sub 1, x sub 2, até x sub 10 marcados
como traços.}
\caption{Ilustração da convergência.
Acima, mostramos os dez primeiros pontos da sequência como um gráfico,
com $M$ e o intervalo em torno do limite $x$ indicados.
Abaixo, os pontos da mesma sequência estão marcados na reta
real.\label{figsequenceconvergence}}
\end{myfigureht}

Quando escrevemos $\lim_{n\to\infty} x_n = x$ para algum número real $x$, estamos afirmando duas
coisas: primeiro, que $\{ x_n \}_{n=1}^\infty$ é convergente; segundo, que seu limite é
$x$.

A definição acima é uma das definições mais importantes da análise,
e é essencial compreendê-la perfeitamente. O ponto-chave da
definição é que, dado \emph{qualquer} $\epsilon > 0$, podemos encontrar um $M$. O
valor de $M$ pode depender de $\epsilon$; por isso, só escolhemos $M$ depois de conhecer
$\epsilon$. Vamos ilustrar a convergência com alguns exemplos.

\begin{example}
A sequência constante $1,1,1,1,\ldots$ converge para 1. Para
todo $\epsilon > 0$, escolha $M = 1$. Isto é, $\sabs{x_n-x} = \sabs{1-1} < \epsilon$ para todo $n$.
\end{example}
...
\end{equation*}
Demonstração: Dado um $\epsilon > 0$, encontre um $M \in \N$ tal que
$0 < \nicefrac{1}{M} < \epsilon$
...
\begin{example}
A sequência $\bigl\{ {(-1)}^n \bigr\}_{n=1}^\infty$ é divergente. Demonstração: Se
houvesse um limite $x$, então, para $\epsilon = \frac{1}{2}$, esperaríamos um $M$ que
satisfizesse a definição. Suponhamos que
esse $M$ exista.
Então, para um índice par com $n \geq M$, calculamos
\begin{equation*}
...
A demonstração desta proposição apresenta uma técnica útil em
análise. Muitas demonstrações seguem o mesmo esquema geral. Queremos
mostrar que certa quantidade é zero. Escrevemos essa quantidade usando a
desigualdade triangular como a soma de duas outras quantidades e estimamos cada uma
por números arbitrariamente pequenos.

\begin{proof}
%NOTE: should be word for word the same as 7.3.3
Suponhamos que $\{ x_n \}_{n=1}^\infty$ tenha limites $x$ e $y$.
Tomemos um $\epsilon > 0$ arbitrário.
Pela definição, encontramos um $M_1$ tal que, para todo $n \geq M_1$,
$\sabs{x_n-x} < \nicefrac{\epsilon}{2}$. De modo semelhante, encontramos um $M_2$
tal que, para todo $n \geq M_2$,
$\sabs{x_n-y} < \nicefrac{\epsilon}{2}$.
Agora tomemos um $n$ tal que $n \geq M_1$ e também $n \geq M_2$, e estimemos
\begin{equation*}
\begin{split}
\sabs{y-x}
& =
...
\end{split}
\end{equation*}
Como $\sabs{y-x} < \epsilon$ para todo $\epsilon > 0$, temos $\sabs{y-x} = 0$
e $y=x$. Portanto, o limite (se existir) é único.
\end{proof}
...
\begin{proof}
Suponhamos que $\{ x_n \}_{n=1}^\infty$ convirja para $x$. Então existe um $M \in \N$
...
\begin{example}
Mostremos que $\left\{ \frac{n^2+1}{n^2+n} \right\}_{n=1}^\infty$ converge e que
\begin{equation*}
\lim_{n\to\infty} \frac{n^2+1}{n^2+n} = 1 .
\end{equation*}

Dado $\epsilon > 0$,
encontremos $M \in \N$ tal que $\frac{1}{M} < \epsilon$. Então, para todo $n \geq
M$,
\begin{equation*}
\begin{split}
...
descartar algumas informações a fim de conseguir uma estimativa mais simples.
\end{example}

\subsection{Sequências monótonas}

O tipo mais simples de sequência é uma sequência monótona. Verificar que
uma sequência monótona converge é tão fácil quanto verificar que ela é limitada.
Também é fácil encontrar
o limite de uma sequência monótona convergente,
desde que possamos encontrar o supremo ou o ínfimo
de um conjunto enumerável de números.

\begin{defn}
Uma sequência $\{ x_n \}_{n=1}^\infty$ é \emph{monótona crescente}\index{sequência monótona crescente} se $x_n \leq x_{n+1}$ para todo $n \in \N$.
%
Uma sequência $\{ x_n \}_{n=1}^\infty$ é \emph{monótona decrescente}\index{sequência monótona decrescente} se $x_n \geq x_{n+1}$ para todo $n \in \N$.
%
Se uma sequência é monótona crescente ou monótona decrescente, podemos
simplesmente dizer que ela é \emph{monótona}\index{sequência monótona}.\footnote{Alguns
autores usam a palavra \emph{monotônica}\index{sequência monotônica}.}
\end{defn}

Por exemplo,
$\{ n \}_{n=1}^\infty$ é monótona crescente,
$\{ \nicefrac{1}{n} \}_{n=1}^\infty$ é monótona decrescente,
a sequência constante $\{ 1 \}_{n=1}^\infty$ é ao mesmo tempo monótona crescente e monótona
decrescente, e $\bigl\{ {(-1)}^n \bigr\}_{n=1}^\infty$ não é monótona.
Os primeiros termos de uma sequência monótona crescente de exemplo
aparecem na
\figureref{figsequenceincreasing}.

\begin{myfigureht}
\myincludegraphics{sequence-increasing}{%
Um gráfico dos primeiros termos de uma sequência, com o eixo horizontal
representando o índice. Cada valor seguinte da sequência é igual ou
maior que o anterior; por isso, os pontos que representam os valores sobem à
medida que avançamos para a direita no gráfico.}
\caption{Os primeiros termos de uma sequência monótona crescente representados
em um gráfico.\label{figsequenceincreasing}}
\end{myfigureht}

\begin{thm}[Teorema da convergência monótona] \label{thm:monotoneconv}
\index{teorema da convergência monótona}%
Uma sequência monótona $\{ x_n \}_{n=1}^\infty$ é limitada se, e somente se, é convergente.

Além disso, se $\{ x_n \}_{n=1}^\infty$ é monótona crescente e limitada, então
\begin{equation*}
\lim_{n\to \infty} x_n = \sup \{ x_n : n \in \N \} .
\end{equation*}
Se $\{ x_n \}_{n=1}^\infty$ é monótona decrescente e limitada, então
\begin{equation*}
...
\begin{proof}
Consideremos uma sequência monótona crescente $\{ x_n \}_{n=1}^\infty$. Suponhamos
primeiro que a sequência seja limitada,
isto é,
que o conjunto $\{ x_n : n \in  \N \}$ seja limitado. Seja
\begin{equation*}
x \coloneqq \sup \{ x_n : n \in \N \} .
\end{equation*}
Seja $\epsilon > 0$ arbitrário. Como $x$ é o supremo,
deve existir pelo menos um $M \in \N$ tal que $x_{M} > x-\epsilon$.
Como $\{ x_n \}_{n=1}^\infty$ é monótona crescente,
é fácil ver (por \hyperref[induction:thm]{indução}) que
$x_n \geq x_{M}$ para todo $n \geq M$. Portanto, para todo $n \geq M$,
\begin{equation*}
\sabs{x_n-x} = x-x_n \leq x-x_{M} < \epsilon  .
\end{equation*}
Logo, $\{ x_n \}_{n=1}^\infty$ converge para $x$.
Portanto, toda sequência monótona crescente e limitada converge.
Na outra direção, já demonstramos que toda sequência convergente
é limitada.

A demonstração para sequências monótonas decrescentes fica como exercício.
\end{proof}

Uma sequência monótona crescente $\{ x_n \}_{n=1}^\infty$
é sempre limitada inferiormente, pois $x_1 \leq x_2 \leq \cdots \leq x_n$
para qualquer $n$; assim, $x_1$ é uma cota inferior. Portanto, para verificar se uma
sequência monótona crescente é limitada, basta verificar se
ela é limitada superiormente. De modo semelhante, uma sequência monótona decrescente
é sempre limitada superiormente; portanto, basta verificar se é
limitada inferiormente.

\begin{example}
Consideremos a sequência $\bigl\{ \frac{1}{\sqrt{n}} \bigr\}_{n=1}^\infty$.

A sequência é
limitada inferiormente, pois
$\frac{1}{\sqrt{n}} > 0$ para todo $n \in \N$.
Mostremos que ela é monótona decrescente. Começamos com
$\sqrt{n+1} \geq \sqrt{n}$ (por que isso é verdade?). Dessa desigualdade
obtemos
\begin{equation*}
\frac{1}{\sqrt{n+1}} \leq \frac{1}{\sqrt{n}} .
\end{equation*}
Logo, a sequência é monótona decrescente e limitada inferiormente (portanto,
limitada). Pelo \thmref{thm:monotoneconv}, concluímos que a sequência é
convergente e que
\begin{equation*}
...
\end{equation*}
Já sabemos que o ínfimo é maior ou igual a 0, pois
0 é uma cota inferior. Tomemos um número $b \geq 0$ tal
que $b \leq \frac{1}{\sqrt{n}}$ para todo $n$. Elevamos ambos os lados ao quadrado e obtemos
\begin{equation*}
b^2 \leq \frac{1}{n} \qquad \text{para todo } n \in \N.
\end{equation*}
Já vimos que isso implica $b^2 \leq 0$ (uma consequência
da \hyperref[thm:arch:i]{propriedade arquimediana}). Como $b^2 \geq 0$ também,
temos $b^2 = 0$,
e, portanto, $b = 0$.
Logo, $b=0$ é a maior cota inferior, e $\lim\limits_{n\to\infty} \frac{1}{\sqrt{n}} = 0$.
\end{example}

\begin{example}
Um alerta: demonstrar que uma sequência monótona é limitada
para poder aplicar o \thmref{thm:monotoneconv} pode ser difícil.
A sequência
$\{ 1 + \nicefrac{1}{2} + \cdots + \nicefrac{1}{n} \}_{n=1}^\infty$
é monótona crescente,
cresce lentamente e, na verdade, cresce cada vez mais devagar à medida que $n$ aumenta.
Veremos, ao estudar séries,
que essa sequência não tem cota superior e, portanto, não converge. Não é
de forma alguma óbvio que ela não tenha cota superior.
\end{example}

Um exemplo comum de sequência monótona surge na seguinte
proposição. A demonstração fica como exercício.

\begin{prop} \label{prop:supinfseq}
Seja $S \subset \R$ um conjunto não vazio e limitado.
Então existem sequências monótonas
$\{ x_n \}_{n=1}^\infty$ e $\{ y_n \}_{n=1}^\infty$ tais que $x_n, y_n \in S$ e
\begin{equation*}
...
\subsection{Caudas de sequências}

\begin{defn}
Dada uma sequência $\{ x_n \}_{n=1}^\infty$,
a \emph{$K$-ésima cauda} (com $K \in \N$),
ou simplesmente a
\emph{cauda}\index{cauda de uma sequência} de
$\{ x_n \}_{n=1}^\infty$, é a sequência que começa no termo de índice $K+1$, geralmente escrita como
\begin{equation*}
\{ x_{n+K} \}_{n=1}^\infty
...
Por exemplo, a cauda de ordem $4$ de $\{ \nicefrac{1}{n} \}_{n=1}^\infty$ é
$\nicefrac{1}{5}, \nicefrac{1}{6}, \nicefrac{1}{7}, \nicefrac{1}{8},
\ldots$. A cauda de ordem $0$ de uma sequência é a própria sequência.
...
A $K$-ésima cauda $\{ x_{n+K} \}_{n=1}^\infty$ converge para todo $K \in \N$.
\item \label{prop:ktail:iii}
A $K$-ésima cauda $\{ x_{n+K} \}_{n=1}^\infty$ converge para algum $K \in \N$.
\end{enumerate}
Além disso, se algum (e, portanto, todos) dos limites existe, então, para todo $K \in \N$,
\begin{equation*}
...
\text{\ref{prop:ktail:i}} \ar[ur, Rightarrow, "\text{demonstrar}"] & &
\text{\ref{prop:ktail:iii}} \ar[ll, Rightarrow, "\text{demonstrar}"]
\end{tikzcd}
\end{equation*}
%mbxENDIGNORE
...
%mbxlatex está rotulada como "demonstrar". A segunda seta de (ii) para (iii)
%mbxlatex não está rotulada. A terceira seta de (iii) para (i) está
%mbxlatex rotulada como "demonstrar".}
%mbxlatex \end{center}
...
Suponhamos que $\{x_n \}_{n=1}^\infty$ convirja para algum $x \in \R$.
Seja $K \in \N$ arbitrário e
definamos $y_n \coloneqq x_{n+K}$. Queremos mostrar que $\{ y_n \}_{n=1}^\infty$ converge
para $x$.
Dado um $\epsilon > 0$, existe um $M \in \N$ tal que
$\sabs{x-x_n} < \epsilon$ para todo $n \geq M$.
Note que $n \geq M$ implica $n+K \geq M$. Portanto, para
todo $n \geq M$, temos
\begin{equation*}
...
Passemos ao caso em que \ref{prop:ktail:iii} implica \ref{prop:ktail:i}.
Seja $K \in \N$ dado, definamos
$y_n \coloneqq x_{n+K}$ e suponhamos que $\{ y_n \}_{n=1}^\infty$ convirja para $x \in \R$.
Isto é, dado um $\epsilon > 0$, existe um $M' \in \N$ tal que
$\sabs{x-y_n} < \epsilon$ para todo $n \geq M'$.
Seja $M \coloneqq M'+K$. Então $n \geq M$ implica $n-K \geq M'$.
Assim, sempre que $n \geq M$, temos
\begin{equation*}
...
No fim das contas, o limite não depende de como a sequência começa; depende apenas
da cauda. O início da sequência
pode ser arbitrário.

Por exemplo, a sequência definida por $x_n \coloneqq \frac{n}{n^2+16}$ é decrescente
se começarmos em $n=4$ (antes disso, é crescente). Isto é:
$\{ x_n \}_{n=1}^\infty =
\nicefrac{1}{17},
\nicefrac{1}{10},
\nicefrac{3}{25},
...
\nicefrac{5}{58} > \ldots .
\end{equation*}
Se descartarmos os três primeiros termos
e considerarmos a cauda de ordem 3, ela será decrescente. A demonstração fica como exercício. Como a cauda de ordem 3
é monótona e limitada inferiormente por zero, ela é convergente e, portanto, a sequência também converge.
...
Seja $\{ n_i \}_{i=1}^\infty$ uma sequência estritamente crescente de números naturais,
isto é, $n_i < n_{i+1}$ para todo $i \in \N$ (em outras palavras, $n_1 < n_2 < n_3 < \cdots$).
A sequência
\begin{equation*}
\{ x_{n_i} \}_{i=1}^\infty
\end{equation*}
...
$\{ \nicefrac{1}{n} \}_{n=1}^\infty
= 1, \nicefrac{1}{2},\nicefrac{1}{3},\nicefrac{1}{4},\ldots$.
Para entender como essas duas
sequências se encaixam na definição, tomemos $n_i \coloneqq 3i$;
isto é, $\{ n_i \}_{i=1}^\infty$ é a
sequência $3,6,9,12,\ldots$.
Os termos $x_{n_i}$ da
subsequência precisam vir da sequência original. Portanto, $1,0,\nicefrac{1}{3},0,
\nicefrac{1}{5},\ldots$
não é uma subsequência de $\{ \nicefrac{1}{n} \}_{n=1}^\infty$. Da mesma forma, a ordem
deve ser preservada:
a sequência $1,\nicefrac{1}{3},\nicefrac{1}{2},\nicefrac{1}{5},\ldots$
não é uma subsequência de $\{ \nicefrac{1}{n} \}_{n=1}^\infty$.

Uma cauda de uma sequência é um tipo especial de subsequência. Para uma
subsequência qualquer, temos a seguinte proposição sobre convergência,
que é mais fraca do que a afirmação correspondente para caudas.

\begin{prop} \label{prop:seqtosubseq}
Se $\{ x_n \}_{n=1}^\infty$ é convergente,
então toda subsequência $\{ x_{n_i} \}_{i=1}^\infty$ também é convergente e
\begin{equation*}
\lim_{n\to \infty} x_n =
...
\begin{proof}
Suponhamos que $\lim_{n\to \infty} x_n = x$. Então, para todo
$\epsilon > 0$, existe um $M \in \N$ tal que, para todo $n \geq M$,
\begin{equation*}
\sabs{x_n - x} < \epsilon .
\end{equation*}
Não é difícil demonstrar (faça isso!) por \hyperref[induction:thm]{indução} que
$n_i \geq i$ para todo $i \in \N$. Portanto, $i \geq M$ implica $n_i \geq M$. Assim,
para todo $i \geq M$,
\begin{equation*}
...
\begin{example}
A existência de uma subsequência convergente não implica
que a própria sequência seja convergente.
Consideremos a sequência $0,1,0,1,0,1,\ldots$. Isto é,
...
$\{ x_{2i+1} \}_{i=1}^\infty$ converge para 0. Compare com \propref{seqconvsubseqconv:prop}.
\end{example}

\subsection{Exercícios}

\begin{exnote}
Nos exercícios a seguir, sinta-se à vontade para usar o que sabe de cálculo para
encontrar o limite, se existir. Mas você deve \emph{demonstrar}
que
...
A sequência
$\{ 3n \}_{n=1}^\infty$
é limitada? Demonstre ou refute.
\end{exercise}

\begin{exercise}
A sequência
$\{ n \}_{n=1}^\infty$
é convergente? Em caso afirmativo, qual é o limite?
\end{exercise}
...
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência.
\begin{enumerate}[a)]
\item Mostre que $\lim\limits_{n\to\infty} x_n = 0$ (isto é, o limite existe e é zero)
se, e somente se, $\lim\limits_{n\to\infty} \sabs{x_n} = 0$.
...
$\left\{ \dfrac{1}{\sqrt[3]{n}} \right\}_{n=1}^\infty$ é monótona e limitada. Em seguida, use
\thmref{thm:monotoneconv} para encontrar seu limite.
\end{exercise}

\begin{exercise}
Mostre que a sequência
$\left\{ \dfrac{n+1}{n} \right\}_{n=1}^\infty$
é monótona e limitada. Em seguida, use
\thmref{thm:monotoneconv} para encontrar seu limite.
\end{exercise}

\begin{exercise}
Conclua a demonstração do \thmref{thm:monotoneconv} para sequências monótonas decrescentes.
\end{exercise}

\begin{exercise}
Demonstre \propref{prop:supinfseq}.
\end{exercise}

\begin{exercise}
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência monótona convergente. Suponha
que exista um $k \in \N$ tal que
\begin{equation*}
...
\end{equation*}
\begin{enumerate}[a)]
\item A sequência é limitada? (demonstre ou refute)
\item Existe alguma subsequência convergente? Em caso afirmativo, encontre-a.
\end{enumerate}
\end{exercise}
...
\pagebreak[2]
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência.
Suponha que haja duas subsequências convergentes $\{ x_{n_i} \}_{i=1}^\infty$ e
$\{ x_{m_i} \}_{i=1}^\infty$. Suponha que
\begin{equation*}
\lim_{i\to\infty} x_{n_i} = a
\qquad \text{e} \qquad
\lim_{i\to\infty} x_{m_i} = b,
\end{equation*}
em que $a \neq b$. Demonstre que $\{ x_n \}_{n=1}^\infty$ não é convergente sem
usar \propref{prop:seqtosubseq}.
\end{exercise}

\begin{exercise}[Desafiador]
Encontre uma sequência $\{ x_n \}_{n=1}^\infty$ tal que, para todo $y \in \R$, exista uma
subsequência $\{ x_{n_i} \}_{i=1}^\infty$ que convirja para $y$.
\end{exercise}

\begin{exercise}[Fácil]
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência e $x \in \R$.
Suponha que, para todo $\epsilon > 0$, exista um $M$ tal que
$\sabs{x_n-x} \leq \epsilon$ para todo $n \geq M$.
Mostre que $\lim\limits_{n\to\infty} x_n = x$.
\end{exercise}

\begin{exercise}[Fácil]
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência e $x \in \R$ tal que
exista um $k \in \N$ para o qual, para todo $n \geq k$,
$x_n = x$. Demonstre que $\{ x_n \}_{n=1}^\infty$ converge para $x$.
\end{exercise}

\begin{exercise}
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência e
definamos uma sequência $\{ y_n \}_{n=1}^\infty$ por
$y_{2k} \coloneqq x_{k^2}$ e $y_{2k-1} \coloneqq x_k$ para todo $k \in \N$.
Demonstre que $\{ x_n \}_{n=1}^\infty$ converge se, e somente se,
$\{ y_n \}_{n=1}^\infty$ converge.
Além disso, demonstre que, se convergirem, então
$\lim\limits_{n\to\infty} x_n = \lim\limits_{n\to\infty} y_n$.
\end{exercise}

\begin{exercise}
Mostre que a cauda de ordem 3 da sequência definida por $x_n \coloneqq \frac{n}{n^2+16}$ é
monótona decrescente. Dica: suponha $n \geq m \geq 4$ e considere o
numerador da expressão $x_n-x_m$.
\end{exercise}

\begin{exercise}
Suponha que $\{ x_n \}_{n=1}^\infty$ seja uma sequência tal que
as subsequências $\{ x_{2n} \}_{n=1}^\infty$, $\{ x_{2n-1} \}_{n=1}^\infty$ e
$\{ x_{3n} \}_{n=1}^\infty$ convirjam. Mostre que $\{ x_n \}_{n=1}^\infty$ converge.
\end{exercise}

\begin{exercise}
Suponha que $\{ x_n \}_{n=1}^\infty$ seja uma sequência monótona crescente que
tenha uma subsequência convergente.
Mostre que $\{ x_n \}_{n=1}^\infty$ converge.
Observação: Assim, \propref{prop:seqtosubseq} é um \myquote{se, e somente se} para sequências monótonas.
\end{exercise}
...
A basic lemma about limits and inequalities is what is called the squeeze lemma.
...
A diagram of the real line.  Points are marked on the line
in order from left to right: x minus epsilon, a sub n, x, x sub n,
b sub n, and x plus epsilon.  Arrows emphasize that the distance
from x minus epsilon to x is epsilon and that the distance
from x to x plus epsilon is also epsilon.}
...
for any $n \geq \max\{ M_1, M_2 \}$, we have
...
The proof of \ref{prop:contalg:ii} is almost identical and is left as an
exercise.
...
Finally, we examine
\ref{prop:contalg:iv}.
We prove the following simpler claim:
...
One such class is recursively defined sequences, that is, sequences where
...
$\bigl\{ \sabs{ x_n - x } \bigr\}_{n=1}^\infty$ converges to zero.
...
In other words, studying when a sequence has a limit is 
the same as studying when another sequence goes to zero.
It may be hard to decide if a particular sequence converges, but
for some sequences there exist easy-to-apply tests for convergence.
Let us see one such test.
First, we compute the limit of a certain specific sequence.
...
\begin{proof}
First consider $c < 1$.  As $c > 0$, we get $c^n > 0$ for all $n \in \N$ by
\hyperref[induction:thm]{induction}.  Then $c < 1$ implies that
$c^{n+1} < c^n$ for all $n$.
The sequence $\{ c^n \}_{n=1}^\infty$ is
thus
bounded below and
decreasing.  Hence, it is convergent.
...
As $\nicefrac{1}{c} < 1$, the sequence $\bigl\{ {(\nicefrac{1}{c})}^n \bigr\}_{n=1}^\infty$
converges to $0$.
...
ratio of the $(n+1)$th term to the $n$th term is $c$.  We 
...
Suppose $L < 1$.
As
$\frac{\sabs{x_{n+1}}}{\sabs{x_n}} \geq 0$ for all $n$, we have $L \geq 0$.  Pick
$r$ such that $L < r < 1$.
...
A diagram of the real line with points L on the left, then
a little to the right a point r and further to the right the point 1.
shorter lines mark points congregating around L and all less
than r.}
...
$n < {(1+\epsilon)}^n$, or 
$n^{1/n} < 1+\epsilon$.  As $n \geq 1$, we find $n^{1/n} \geq 1$, and
...
If $\{ x_n \}_{n=1}^\infty$ is a sequence
...
\end{equation*}
Hint: First, handle the limit being zero.
Next, find an expression $q$ such that $\frac{x_n^{1/k}-x^{1/k}}{x_n-x} =
\frac{1}{q}$.
\end{exercise}
...
some $x \neq 0$, and $\{ a_n b_n \}_{n=1}^\infty$ is not convergent.
\end{enumerate}
\end{exercise}
\end{samepage}
...
We define what we call limit superior and limit inferior
of a bounded sequence, which are the largest and smallest
possible limits of subsequences.
...
in this fashion we
get \emph{\myindex{limit superior}}\index{limsup} and
...
A graph of an example sequence x sub n in dots
for 50 points, that is, n from 1 to 50.
In the middle there are two horizontal dashed lines,
the upper one is marked as lim sup as n goes to infinity of x sub n
and
the lower one is marked as lim inf as n goes to infinity of x sub n.
The dots land both above and below the lines but they seem
to congregate closer to the region between the two lines on the 
right hand side.  Two extra sequences are marked,
on the bottom with diamonds, is a sequence that is less than or equal
to all the dots and constant, until it hits a dot.  Then it goes up
and is constant until it hits a dot, and this keeps repeating.
So it looks like a series of horizontal rows ending where the
diamond as at the same place as a dot, and all the other dots are
above the diamonds.
All the diamonds are below the lower dashed line.
A similar pattern but reversed is with circles from above,
that is a row of circles until it hits a dot, then it goes down
and repeats, and all the circles are above or on the dots.
All the circles are above the upper dashed line.}
...
The same marking as in the previous figure but now for a very specific
sequence.  The lower dashed line (the lim inf) is the horizontal axis
and the upper dashed line is roughly in the middle of the picture.
The first dot is high up above the upper dashed line
and a circle is at the same
place, while the diamond is on the horizontal axis.
In fact, all the diamonds
are on the horizontal axis.
The second dot is on the horizontal axis, but the circle is above
the upper dashed line but lower than the first dot.
The third dot is at the same height as the second circle
and the third circle is at the same spot.
The pattern now keeps repeating as the dots for odd n
and the circles get closer and closer to the upper dashed line
from above.}
...
subsequences of $\{ x_n \}_{n=1}^\infty$, nor need they
...
for every $n$, the set
$\{ x_k : k \geq n \}$ consists of $0$ and positive numbers.
So,
\begin{equation*}
...
For example, $S \coloneqq \{ \nicefrac{1}{n} : n \in \N \}$ has a unique cluster point $0$, but $0 \notin S$.
...
a sequence such that $x_{n+1}-x_n = \frac{1}{n+1}$,
...
there exists a positive $C < 1$ and for all $n \geq 2$,
\begin{equation*}
...
Show that $\{ x_n \}_{n=1}^\infty$ is Cauchy.\footnote{After proving that
...
\begin{exercise}
Suppose 
$\{ x_n \}_{n=1}^\infty$ is a sequence such that for every $\ell$ and $k$,
we have
$\sabs{x_\ell-x_k} \leq y_\ell + z_k$, where
$\{ y_n \}_{n=1}^\infty$ and 
$\{ z_n \}_{n=1}^\infty$ are sequences that converge to $0$.
Show that 
$\{ x_n \}_{n=1}^\infty$ is Cauchy.
\end{exercise}
...
A diagram of the real line.  The point 0 is on the left and the point
1 is on the right.  A distance of one half is marked from the 0 to
one half.  Then the distance of one quarter is marked from one half
to the point at 3 quarters.  Then a distance of one eighth is marked from
3 quarters to 7 eighths.  Also marked is the distance one half plus one quarter plus one
eighth, which puts together the three aforementioned distances.
Lastly the remaining distance of one eighth from 7 eighths to 1 is also
marked.}
...
The geometric series is one of the most important series, and in fact it is
one of the few series for which we can so explicitly find the limit.

\medskip

\pagebreak[2]
As with sequences, we can talk about a \emph{\myindex{tail of a series}}.
...
So when $x_n \geq 0$ for all $n$, we have the
inequality
\begin{equation*}
\sum_{n=1}^k x_n \leq
\sum_{n=1}^\infty x_n .
...
series together easily.  Conditionally convergent series,
on the other hand,
...
As $p$ is positive, $2^p < 3^p$, and hence
...
tells us what the sum converges to.  They only tell us that a limit
...
we get the Riemann $\zeta$ (zeta) function.  Understanding the
