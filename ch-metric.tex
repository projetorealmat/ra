...
\sectionnewpage
\section{Teorema do ponto fixo e novamente o teorema de Picard}
\label{sec:metpicard}

%mbxINTROSUBSECTION

\sectionnotes{1 aula (opcional, não requer \sectionref{sec:picard})}

Nesta seção demonstramos o teorema do ponto fixo para
aplicações contrativas. Como aplicação, demonstramos o teorema de Picard,
que demonstramos sem espaços métricos na \sectionref{sec:picard}.
A demonstração apresentada aqui é semelhante, mas fica muito
mais simples com espaços métricos e com o teorema do ponto fixo.
...
Sejam $(X,d_X)$ e $(Y,d_Y)$ espaços métricos.
Uma aplicação
$\varphi \colon X \to Y$ é uma \emph{\myindex{contração}}
(ou aplicação contrativa) se é uma aplicação $k$-Lipschitz para algum $k < 1$,
isto é, se existe um $k < 1$ tal que
\begin{equation*}
...
Dada uma aplicação $\varphi \colon X \to X$, dizemos que um ponto $x \in X$ é
um \emph{\myindex{ponto fixo}}
se $\varphi(x)=x$.
\end{defn}

\begin{thm}%
[Princípio da contração\index{princípio da contração}
ou \myindex{teorema do ponto fixo de Banach}\index{teorema do ponto fixo}%
\footnote{Nomeado em homenagem ao matemático polonês
\href{https://en.wikipedia.org/wiki/Stefan_Banach}{Stefan Banach}
(1892--1945), que enunciou o teorema pela primeira vez em 1922.}]
\label{thm:contr}
Seja $(X,d)$ um espaço métrico completo não vazio e seja $\varphi \colon X \to X$ uma
contração.
Então $\varphi$ tem um único ponto fixo.
\end{thm}

As hipóteses de \emph{completude} e de \emph{contração} são necessárias.
Veja \exerciseref{exercise:nofixedpoint}.

\begin{proof}
Escolha $x_0 \in X$.
Defina uma sequência $\{ x_n \}_{n=1}^\infty$ por $x_{n+1} \coloneqq \varphi(x_n)$. Então
\begin{equation*}
...
%\leq k^n d(x_1,x_0) .
\end{equation*}
Repetindo esse argumento $n$ vezes, obtemos $d(x_{n+1},x_n) \leq k^n d(x_1,x_0)$.
Para $m > n$,
\begin{equation*}
\begin{split}
d(x_m,x_n)
...
\end{split}
\end{equation*}
Em particular, a sequência é de Cauchy (por quê?). Como $X$ é completo,
definimos $x \coloneqq \lim_{n\to\infty} x_n$ e afirmamos que $x$
é o único ponto fixo.

É ponto fixo? A função $\varphi$ é uma contração,
portanto é contínua:
\begin{equation*}
...
\end{equation*}
É único? Sejam $x$ e $y$ pontos fixos.
\begin{equation*}
d(x,y) = d\bigl(\varphi(x),\varphi(y)\bigr) \leq k\, d(x,y) .
\end{equation*}
Como $k < 1$, a desigualdade implica que $d(x,y) = 0$ e, portanto, $x=y$. O teorema está
demonstrado.
\end{proof}

A demonstração é construtiva. Além de sabermos
que existe um único ponto fixo, sabemos como encontrá-lo. Comece com
qualquer ponto $x_0 \in X$ e, em seguida, itere $\varphi(x_0)$,
$\varphi(\varphi(x_0))$,
$\varphi(\varphi(\varphi(x_0)))$ etc.\ para obter aproximações cada vez melhores.
Também podemos determinar quão longe estamos
do ponto fixo.
Veja os exercícios. Portanto, a ideia da demonstração
é útil em aplicações do mundo real.

\subsection{Teorema de Picard}

Começamos pelo espaço métrico em que aplicaremos o
teorema do ponto fixo:
o espaço $C\bigl([a,b],\R\bigr)$ de \exampleref{example:msC01},
formado pelas funções contínuas $f \colon [a,b] \to \R$, com a métrica
\begin{equation*}
d(f,g) \coloneqq \snorm{f-g}_{[a,b]} = \sup_{x \in [a,b]} \babs{f(x)-g(x)} .
\end{equation*}
Convergir nessa métrica equivale a convergir na norma uniforme, ou seja,
a convergir uniformemente. Portanto,
$C\bigl([a,b],\R\bigr)$ é um espaço métrico completo;
veja \propref{prop:CabRcomplete}.

\medskip

Consideremos agora a equação diferencial ordinária
\begin{equation*}
\frac{dy}{dx} = F(x,y) .
\end{equation*}
Dados $x_0,y_0$, queremos uma função $y=f(x)$ tal que
$f(x_0) = y_0$ e
\begin{equation*}
f'(x) = F\bigl(x,f(x)\bigr) .
\end{equation*}
Para evitar a introdução de muitos nomes, escrevemos muitas vezes apenas $y' = F(x,y)$
para a equação
e $y(x)$ para a solução.

O exemplo mais simples é a equação $y' = y$, $y(0) = 1$.
Sua solução é a função exponencial $y(x) = e^x$. Um exemplo um pouco mais complicado
é $y' = -2xy$, $y(0) = 1$, cuja solução é a função gaussiana
$y(x) = e^{-x^2}$.

Uma questão sutil é por quanto tempo a solução existe.
Considere a equação $y' = y^2$, $y(0)=1$. Então $y(x) = \frac{1}{1-x}$ é uma
solução. Embora $F$ seja uma função razoavelmente \myquote{bem-comportada} e, em particular,
esteja definida para todos os valores de $x$ e $y$, a solução \myquote{se torna ilimitada} em $x=1$.
Para mais exemplos relacionados ao teorema de Picard, veja \sectionref{sec:picard}.

Procuraremos a solução em
$C\bigl([a,b],\R\bigr)$, o que pode parecer estranho à primeira vista, pois
estamos procurando uma função diferenciável.
A explicação é que consideramos a
...
\end{equation*}
Para resolver essa equação integral, basta uma função contínua; portanto,
em certo sentido, a tarefa fica mais fácil — temos mais funções candidatas
a testar. Essa maneira de pensar é bastante comum na resolução de equações
diferenciais.

\begin{samepage}
\begin{thm}[Teorema de existência e unicidade de Picard]%
\index{teorema da existência e unicidade}\index{teorema de Picard}
Sejam $I, J \subset \R$ intervalos fechados e limitados,
sejam $I^\circ$ e $J^\circ$ seus interiores, e seja
$(x_0,y_0) \in I^\circ \times J^\circ$.
Suponha que $F \colon I \times J \to \R$ seja contínua
e Lipschitz na segunda variável, isto é, que exista
um $L \in \R$ tal que
\begin{equation*}
\babs{F(x,y) - F(x,z)} \leq L \sabs{y-z}
\qquad \text{para todos } y,z \in J, x \in I .
\end{equation*}
Então existe um $h > 0$ tal que $[x_0-h,x_0+h] \subset I$ e uma única função diferenciável
$f \colon [x_0 - h, x_0 + h] \to J \subset \R$ tal que
\begin{equation*}
...
\begin{proof}
Sem perda de generalidade, suponha que $x_0 =0$ (exercício).
Como $I \times J$ é compacto e
$F$ é contínua, $F$ é limitada.
Escolha então um $M > 0$ tal que
...
\end{equation*}
Note que $[-h,h] \subset I$. Seja
\begin{equation*}
Y \coloneqq \Bigl\{ f \in C\bigl([-h,h],\R\bigr) : f\bigl([-h,h]\bigr) \subset J \Bigr\} .
\end{equation*}
Isto é, $Y$ é o conjunto das funções contínuas em $[-h,h]$ com valores em
$J$, ou seja,
exatamente as funções para as quais $F\bigl(x,f(x)\bigr)$ está definida.
Deixamos como exercício demonstrar que $Y$ é um subconjunto fechado de $C\bigl([-h,h],\R\bigr)$
(pois $J$ é fechado).
O espaço $C\bigl([-h,h],\R\bigr)$ é completo, e
um subconjunto fechado de um espaço métrico completo é completo com
a métrica de subespaço; veja \propref{prop:closedcomplete}. Portanto, $Y$ com a
métrica de subespaço é um espaço métrico completo.
Escreveremos $d(f,g) = \snorm{f-g}_{[-h,h]}$ para essa métrica.

Defina uma aplicação
$T \colon Y \to C\bigl([-h,h],\R\bigr)$ por
\begin{equation*}
T(f)(x)
\coloneqq
...
$T$ está bem definida e que, para $f \in Y$, $T(f)$ de fato pertence a $C\bigl([-h,h],\R\bigr)$.
Sejam $f \in Y$ e $\sabs{x} \leq h$.
Como $F$ é limitada por $M$, temos
\begin{equation*}
\begin{split}
\babs{T(f)(x) - y_0}
...
\end{split}
\end{equation*}
Assim, $T(f)\bigl([-h,h]\bigr) \subset [y_0-\alpha,y_0+\alpha] \subset J$ e
$T(f) \in Y$. Em outras palavras, $T(Y) \subset Y$. A partir daqui,
consideramos $T$ como uma aplicação de $Y$ em $Y$.

Afirmamos que $T \colon Y \to Y$ é uma contração. Primeiro, para $x \in [-h,h]$
e $f,g \in Y$, temos
\begin{equation*}
...
\end{split}
\end{equation*}
Escolhemos $M > 0$ e, portanto,
$\frac{L\alpha}{M+L\alpha} < 1$.
Tomando o supremo em $x \in [-h,h]$ do lado esquerdo da desigualdade acima, obtemos
$d\bigl(T(f),T(g)\bigr) \leq \frac{L\alpha}{M+L\alpha} \, d(f,g)$,
isto é, $T$ é uma contração.

O teorema do ponto fixo (\thmref{thm:contr})
fornece uma única $f \in Y$ tal que $T(f) = f$.
Em outras palavras,
\begin{equation*} %\label{equation:msinteqpicard}
f(x) = y_0 + \int_0^x F\bigl(t,f(t)\bigr)\,dt .
\end{equation*}
Claramente, $f(0) = y_0$.
Pelo teorema fundamental do cálculo (\thmref{thm:FTCv2}),
$f$ é diferenciável e sua derivada é
$F\bigl(x,f(x)\bigr)$.
Funções diferenciáveis são contínuas; portanto,
$f$ é a única função diferenciável $f \colon [-h,h] \to J$
tal que
 $f'(x) = F\bigl(x,f(x)\bigr)$ e $f(0) = y_0$.
\end{proof}
...
\begin{exnote}
Para mais exercícios relacionados ao teorema de Picard, veja \sectionref{sec:picard}.
\end{exnote}

\begin{exercise}
Seja $J$ um intervalo fechado e limitado e
$Y \coloneqq \bigl\{ f \in C\bigl([-h,h],\R\bigr) : f\bigl([-h,h]\bigr) \subset J \bigr\}$.
Demonstre que $Y \subset C\bigl([-h,h],\R\bigr)$ é fechado. Dica: $J$ é fechado.
\end{exercise}

\begin{exercise}
Na demonstração do teorema de Picard,
prove que, se $f \colon [-h,h] \to J$ é contínua, então $F\bigl(t,f(t)\bigr)$
é contínua em $[-h,h]$ como função de $t$. Use isso para demonstrar que
\begin{equation*}
T(f)(x)
\coloneqq
y_0 + \int_0^x F\bigl(t,f(t)\bigr)\,dt
\end{equation*}
está bem definida e que $T(f) \in C\bigl([-h,h],\R\bigr)$.
\end{exercise}


\begin{exercise}
Demonstre que, na demonstração do teorema de Picard,
a afirmação \myquote{sem perda de generalidade, suponha que $x_0 = 0$} está
justificada. Isto é, prove que, se o teorema vale para $x_0 = 0$, então
vale como enunciado.
\end{exercise}

\begin{exercise}
\pagebreak[2]
Seja $F \colon \R \to \R$ definida por
$F(x) \coloneqq kx + b$, onde $0 < k < 1$ e $b \in \R$.
\begin{enumerate}[a)]
\item
Demonstre que $F$ é uma contração.
\item
Encontre o ponto fixo e demonstre diretamente que ele é único.
\end{enumerate}
\end{exercise}

\begin{exercise}
\pagebreak[2]
Seja $f \colon [0,\nicefrac{1}{4}] \to [0,\nicefrac{1}{4}]$ definida por
$f(x) \coloneqq x^2$.
\begin{enumerate}[a)]
\item
Demonstre que $f$
é uma contração e encontre o menor valor de $k$ que funciona na definição.
\item
Encontre o ponto fixo e demonstre diretamente que ele é único.
\end{enumerate}
\end{exercise}
...
\begin{enumerate}[a)]
\item
Encontre um exemplo de contração $f \colon X \to X$
em um espaço métrico incompleto $X$ que não tenha
ponto fixo.
\item
Encontre uma aplicação 1-Lipschitz $f \colon X \to X$ em um espaço métrico completo $X$ que não tenha ponto fixo.
\end{enumerate}
\end{exercise}
\end{samepage}

\begin{exercise}
Considere $y' =y^2$, $y(0)=1$. Use o esquema de iteração
da demonstração do princípio da contração.
Comece com $f_0(x) = 1$. Encontre algumas
iterações (pelo menos até $f_2$). Demonstre que
o limite pontual de $f_n$ é $\frac{1}{1-x}$, isto é, para todo $x$
com $\sabs{x} < h$ para algum $h > 0$,
demonstre que $\lim\limits_{n\to\infty}f_n(x) = \frac{1}{1-x}$.
\end{exercise}

\begin{exercise}
Suponha que $f \colon X \to X$ seja uma contração com $k < 1$. Considere a iteração $x_{n+1} \coloneqq f(x_n)$, como na demonstração do teorema do ponto fixo,
e seja $x$ o ponto fixo de $f$.
\begin{enumerate}[a)]
\item
Demonstre que $d(x,x_n) \leq k^n d(x_1,x_0) \frac{1}{1-k}$ para todo $n \in \N$.
\item
Suponha que $d(y_1,y_2) \leq 16$ para todos $y_1,y_2 \in X$ e que $k=
\nicefrac{1}{2}$. Encontre um $N$ tal que, começando em qualquer ponto dado $x_0 \in X$,
$d(x,x_n) \leq 2^{-16}$ para todo $n \geq N$.
\end{enumerate}
\end{exercise}

\begin{exercise}
Seja $f(x) \coloneqq x-\frac{x^2-2}{2x}$ (talvez você reconheça o método de Newton para
$\sqrt{2}$).
\begin{enumerate}[a)]
\item
Demonstre que $f\bigl([1,\infty)\bigr) \subset [1,\infty)$.
\item
Demonstre que $f \colon [1,\infty) \to [1,\infty)$ é uma contração.
\item
Mostre que o teorema do ponto fixo se aplica,
encontre o único $x \geq 1$ tal que $f(x) = x$
e demonstre que $x = \sqrt{2}$.
Observação: Em particular, a técnica da demonstração do teorema
pode ser usada para aproximar $\sqrt{2}$.
\end{enumerate}
\end{exercise}

\begin{exercise}
Suponha que $f \colon X \to X$ seja uma contração e que $(X,d)$ seja um espaço métrico
com a métrica discreta, isto é, $d(x,y) = 1$ sempre que $x \neq y$.
Demonstre que $f$ é constante, isto é,
existe um $c \in X$ tal que $f(x) = c$ para todo $x \in X$.
\end{exercise}

\begin{exercise}
Seja $(X,d)$ um espaço métrico completo não vazio,
seja $f \colon X \to X$ uma aplicação, e denote
por $f^n$ a $n$-ésima iterada de $f$. Suponha que,
para todo $n$, exista um $k_n > 0$ tal que
$d\bigl(f^n(x),f^n(y)\bigr) \leq k_n \, d(x,y)$
para todos $x,y \in X$,
onde $\sum_{n=1}^\infty k_n < \infty$.
Demonstre que $f$ tem um único ponto fixo em $X$.
\end{exercise}
