...
we define the \emph{\myindex{topology}}.
That is,
we define the notion of open and closed sets.
And before that,
...
A region in the plane marked in light gray,
with a dotted boundary and labeled as V.
Inside the region there is a dotted
circle with center
at a point marked x of radius delta and labeled
as B of x comma delta.  The circle with its inside
shaded darker gray lies completely inside the
lighter gray region.}
...
A shaded disc with a dotted boundary of radius delta 
is centered at x and labeled as B of x comma delta.
There is another disc that lies completely inside the bigger disc
touching its boundary from the inside and is centered
at y and is of radius alpha.
A third point z is inside the
smaller disc.  The points x, y, and z give a
dashed triangle.}
...
The set $[0,\nicefrac{1}{2})$ is
an open ball in $[0,1]$, e, portanto, $[0,\nicefrac{1}{2})$ is
...
only subsets of $X$ that are both open and closed (sometimes called
...
Two shaded regions with a solid boundary are both
labeled S.  A dotted line surrounds each shaded
region.
The inside of the first one is marked U sub 1
and the inside of the second one is marked U sub 2.
The insides of the two dotted lines do intersect, but
each shaded region is inside only one of the dotted
lines.}
...
$S$ is connected, we must have that their union is not $S$, so $z \in S$.
...
A diagram of the number line with a point
x marked on the left and point y marked
on the right.  Between x and y
there are points z and w.  An open interval
going from somewhere on the left and past w (so
it contains x, z and w) is marked U sub 1.
An open interval from z going off the diagram
to the right is marked as U sub 2.
a small open interval from z minus delta
to z plus delta that contains w is marked.}
...
A^\circ$, e, portanto, $A^\circ$ is open.
...
A picture near the boundary of a set A which is shaded.
The boundary is given as a solid line with A below
and the complement of A above.  A dotted
circle of radius delta is centered at a point x on
the boundary and is marked as B of x comma delta.}
...
Prove that $\Q$ is
\emph{\myindex{totally separated}}, that is, show 
...
the standard metric $d(x,y)=\sabs{x-y}$ is replaced
...
of $\{ x_n \}_{n=1}^\infty$.
We will prove the limit is unique, and so
we write\glsadd{not:limseq}
\begin{equation*}
...
A dotted circle of radius epsilon centered at p is drawn.
A bunch of points from the beginning of the sequence
are shown for x sub 1 through x sub 10.  The points
x sub 1, x sub 3, x sub 4, x sub 5, and x sub 6 are
outside the circle, but all the other points are inside.}
...
We start by proving the limit is unique.  The proof is almost
...
adapted to
the more general settings of metric spaces.  We must replace $\sabs{x-y}$
...
\end{split}
\end{equation*}
As $d(y,x) < \epsilon$ for all $\epsilon > 0$, we have $d(x,y) = 0$
e, portanto, $y=x$.  Hence, the limit (if it exists) is unique.
\end{proof}
...
\begin{example}
Take $C\bigl([a,b],\R\bigr)$ to be the set of continuous functions with the metric being
...
\R$ (continuous or not)
there is no metric that gives pointwise convergence as the domain $[a,b]$
is uncountable.  The proof of
...
of $p$.  There exists an $\epsilon > 0$ such that $B(p,\epsilon) \subset
...
\begin{enumerate}[a)]
\item
Show that $d(x,y) \coloneqq \min \bigl\{ 1, \sabs{x-y} \bigr\}$ defines a metric on $\R$.
\item
...
Let $d'(x,y) \coloneqq \sum_{\ell=1}^n \sabs{x_\ell-y_\ell}$ and
...
e, portanto, there must be an $x_n \in K$
...
e, portanto, $x \in E$.  Thus $E$ must be compact.
\end{proof}
...
sequences converge, e, portanto, it is no wonder that the topology also
...
\qquad \text{para todo } y,z \in J, x \in I .
\end{equation*}
