...
%\emph{limit of a sequence}\index{limite!de uma sequ\^encia}.
...
\begin{exercise}
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência tal que existe um número positivo $C < 1$ e, para todo $n \geq 2$,
\begin{equation*}
...
\index{série p@série $p$}\index{critério p@critério $p$}
Para $p \in \R$,
a série
\begin{equation*}
...
obtemos a função zeta de Riemann $\zeta$. O estudo do
...
\begin{exercise}
Suponha que a $k$-ésima soma parcial de $\displaystyle \sum_{n=1}^\infty x_n$ seja $s_k = \frac{k}{k+1}$.
Determine a série (isto é, determine $x_n$), demonstre que a série converge e,
em seguida, encontre o limite.
\end{exercise}

\begin{exercise} \label{geometric:exr}
Demonstre a \propref{geometric:prop}; isto é, para $-1 < r < 1$, demonstre
\begin{equation*}
\sum_{n=0}^\infty r^n = \frac{1}{1-r} .
\end{equation*}
Dica: veja o \exampleref{example:geometricsum}.
\end{exercise}
...
também converge.
\item
Encontre um exemplo explícito em que a recíproca não vale.
\end{enumerate}
\end{exercise}
\end{samepage}

\begin{exercise}
Para $i=1,2,\ldots,n$, sejam $\{ x_{i,k} \}_{k=1}^\infty$ $n$
sequências. Suponha que, para cada $i$,
\begin{equation*}
...
Demonstre a seguinte versão mais forte do teste da razão:
seja $\sum_{n=1}^\infty x_n$ uma série.
\begin{enumerate}[a)]
\item
Se existem $N$ e $\rho < 1$ tais que
$\frac{\sabs{x_{n+1}}}{\sabs{x_n}} < \rho$ para todo $n \geq N$,
então a série converge absolutamente.
(Observação: de modo equivalente, a condição pode ser expressa como
...
\begin{exercise}[Desafiador]
Suponha que $\{ x_n \}_{n=1}^\infty$ seja uma sequência decrescente e
que $\sum_{n=1}^\infty x_n$ convirja.
Demonstre que $\displaystyle \lim_{n\to\infty} n x_n = 0$.
\end{exercise}

\begin{exercise}
Mostre que $\displaystyle \sum_{n=1}^\infty \frac{{(-1)}^n}{n}$ converge.
Dica: considere a soma de dois termos consecutivos.
\end{exercise}

\begin{exercise}
\leavevmode
\begin{enumerate}[a)]
\item Demonstre que, se $\sum_{n=1}^\infty x_n$ e $\sum_{n=1}^\infty y_n$ convergem absolutamente, então
$\sum_{n=1}^\infty x_ny_n$ converge absolutamente.
\item Encontre um exemplo explícito em que a recíproca não vale.
\item Encontre um exemplo explícito em que as três séries sejam absolutamente convergentes,
não sejam apenas somas finitas e satisfaçam
$\left(\sum_{n=1}^\infty x_n\right)\left(\sum_{n=1}^\infty y_n\right)
\neq \sum_{n=1}^\infty x_ny_n$. Isto é, mostre que as séries
não são multiplicadas termo a termo.
\end{enumerate}
\end{exercise}

\begin{exercise}
Demonstre a desigualdade triangular para séries:
se $\sum_{n=1}^\infty x_n$ converge absolutamente, então
\begin{equation*}
...
\begin{exercise}
Demonstre o \emph{\myindex{teste da comparação pelo limite}}. Isto é, demonstre que, se
$a_n > 0$ e $b_n > 0$ para todo $n$, e
\begin{equation*}
0 < \lim_{n\to\infty} \frac{a_n}{b_n} < \infty ,
\end{equation*}
então $\sum_{n=1}^\infty a_n$ e $\sum_{n=1}^\infty b_n$ ou convergem ambas, ou divergem ambas.
\end{exercise}

\begin{exercise} \label{exercise:badnocauchy}
Seja $x_n \coloneqq \sum_{i=1}^n \nicefrac{1}{i}$. Mostre que, para todo $k$,
temos
$\displaystyle \lim_{n\to\infty} \sabs{x_{n+k}-x_n} = 0$, mas
$\{ x_n \}_{n=1}^\infty$ não é de Cauchy.
\end{exercise}

\begin{samepage}
\begin{exercise}
Seja $s_k$ a $k$-ésima soma parcial de $\sum_{n=1}^\infty x_n$.
\begin{enumerate}[a)]
\item
Suponha que exista $m \in \N$ tal que $\lim\limits_{k\to\infty}
s_{mk}$ exista e $\lim\limits_{n\to\infty} x_n = 0$. Mostre que
$\sum_{n=1}^\infty x_n$ converge.
\item
Encontre um exemplo em que $\lim\limits_{k\to\infty} s_{2k}$ exista e
$\lim\limits_{n\to\infty} x_n \neq 0$ (e, portanto, $\sum_{n=1}^\infty x_n$ divirja).
\item
(Desafiador) Encontre um exemplo em que $\lim_{n\to\infty} x_n = 0$ e exista
uma subsequência $\{ s_{k_i} \}_{i=1}^\infty$ tal que $\displaystyle \lim_{i\to\infty} s_{k_i}$ exista,
mas $\sum_{n=1}^\infty x_n$ ainda divirja.
\end{enumerate}
\end{exercise}
\end{samepage}

\begin{exercise} \label{exercise:squareseriesconv}
Suponha que $\sum_{n=1}^\infty x_n$ convirja e que $x_n \geq 0$ para todo $n$.
Demonstre que $\sum_{n=1}^\infty x_n^2$ converge.
\end{exercise}

\begin{exercise}[Desafiador] \label{exercise:cauchycondensation}
Suponha que $\{ x_n\}_{n=1}^\infty$ seja uma sequência decrescente de números positivos.
A demonstração da convergência ou divergência da série $p$ se generaliza.
Demonstre o
\emph{\myindex{princípio da condensação de Cauchy}}:
\begin{equation*}
\sum_{n=1}^\infty x_n
\qquad \text{converge se, e somente se,} \qquad
\sum_{n=1}^\infty 2^n x_{2^n} \qquad \text{converge}.
\end{equation*}
\end{exercise}

\begin{exercise}
Use o princípio da condensação de Cauchy
(veja o \exerciseref{exercise:cauchycondensation})
para determinar a convergência ou divergência de
...
\noindent
Observe que apenas as caudas de algumas dessas séries satisfazem as
hipóteses do princípio; justifique por que isso é suficiente.

\noindent
Dica: use, se desejar, a identidade $\ln (2^n) = n \ln 2$.
\end{exercise}

\begin{exercise}[Desafiador]
Demonstre o \emph{\myindex{teorema de Abel}}:

\medskip

\noindent
\emph{\textbf{Teorema.} Suponha que $\sum_{n=1}^\infty x_n$ seja uma série cuja sequência de somas parciais
é limitada, que $\{ \lambda_n \}_{n=1}^\infty$ seja uma sequência tal que
$\lim_{n\to\infty} \lambda_n = 0$ e que
$\sum_{n=1}^\infty \sabs{ \lambda_{n+1} - \lambda_n }$ convirja.
Então $\sum_{n=1}^\infty \lambda_n x_n$ converge.}
\end{exercise}
...
\sectionnewpage
\section{Mais sobre séries}
\label{sec:moreonseries}

%mbxINTROSUBSECTION

\sectionnotes{até 2--3 aulas (opcional; pode ser omitida sem prejuízo ou abordada parcialmente)}

\subsection{Critério da raiz}

Um critério semelhante ao critério da razão é o
\emph{\myindex{critério da raiz}}. A demonstração também é semelhante.
Mais uma vez, a ideia é generalizar o que ocorre com a série geométrica.

\begin{prop}[Critério da raiz]
Seja $\sum_{n=1}^\infty x_n$ uma série e seja
\begin{equation*}
L \coloneqq \limsup_{n\to\infty} {\sabs{x_n}}^{1/n} .
\end{equation*}
\begin{enumerate}[(i)]
\item Se $L < 1$, então $\sum_{n=1}^\infty x_n$ converge absolutamente.
\item Se $L > 1$, então $\sum_{n=1}^\infty x_n$ diverge.
\end{enumerate}
\end{prop}

\begin{proof}
Se $L > 1$, existe\footnote{%
No caso $L=\infty$, veja o \exerciseref{exercise:extendsubseqlimsupinf}.
Como alternativa, observe que, se $L=\infty$, então $\bigl\{ {\sabs{x_{n}}}^{1/n}
\bigr\}_{n=1}^\infty$
é ilimitada e, portanto, o mesmo vale para
$\{ x_n \}_{n=1}^\infty$.}
uma subsequência $\{ x_{n_k} \}_{k=1}^\infty$ tal que
$L = \lim_{k\to\infty} {\sabs{x_{n_k}}}^{1/n_k}$. Seja
$r$ tal que $L > r > 1$.  Existe $M$ tal
que, para todo $k \geq M$, temos 
${\sabs{x_{n_k}}}^{1/n_k} > r > 1$, ou, em outras palavras
$\sabs{x_{n_k}} > r^{n_k} > 1$.
A subsequência 
$\bigl\{ \sabs{x_{n_k}} \bigr\}_{k=1}^\infty$,
e, portanto, também
$\bigl\{ \sabs{x_{n}} \bigr\}_{n=1}^\infty$,
não pode convergir para zero; logo, a série diverge.

Suponha agora que $L < 1$. Escolha $r$ tal que $L < r < 1$.
Pela definição de limite superior,
existe $M$ tal que, para todo $n \geq M$,
\begin{equation*}
\sup \bigl\{ {\sabs{x_k}}^{1/k} : k \geq n \bigr\} < r .
\end{equation*}
Portanto, para todo $n \geq M$,
\begin{equation*}
{\sabs{x_n}}^{1/n} < r , \qquad \text{ou, em outras palavras} \qquad \sabs{x_n} < r^n .
\end{equation*}
Seja $k > M$, e estimemos a $k$-ésima soma parcial:
\begin{equation*}
\sum_{n=1}^k \sabs{x_n} = 
\left( \sum_{n=1}^M \sabs{x_n} \right) + 
\left( \sum_{n=M+1}^k \sabs{x_n} \right)
...
\end{equation*}
Como $0 < r < 1$,
a série geométrica $\sum_{n=M+1}^\infty r^n$ converge para
$\frac{r^{M+1}}{1-r}$. Como todos os termos são positivos,
\begin{equation*}
\sum_{n=1}^k \sabs{x_n} 
\leq
\left( \sum_{n=1}^M \sabs{x_n} \right) + 
\frac{r^{M+1}}{1-r} .
\end{equation*}
Assim, a sequência das somas parciais de $\sum_{n=1}^\infty \sabs{x_n}$ é limitada, e
a série converge. Portanto, $\sum_{n=1}^\infty x_n$ converge absolutamente.
\end{proof}

\subsection{Critério das séries alternadas}

Os critérios que vimos até agora tratam apenas da convergência absoluta. O
critério a seguir fornece muitos exemplos de séries condicionalmente convergentes.

\begin{prop}[Série alternada]
Seja $\{ x_n \}_{n=1}^\infty$ uma sequência monótona decrescente de números reais positivos tal
que $\lim\limits_{n\to\infty} x_n = 0$. Então
\begin{equation*}
\sum_{n=1}^\infty {(-1)}^n x_n \qquad \text{converge.}
\end{equation*}
\end{prop}

\begin{proof}
Seja $s_m \coloneqq \sum_{n=1}^m {(-1)}^n x_n$ a $m$-ésima soma parcial. Então
\begin{equation*}
s_{2k} =
\sum_{n=1}^{2k} {(-1)}^n x_n
=
...
\end{equation*}
A sequência $\{ x_n \}_{n=1}^\infty$ é decrescente, logo $(-x_{2\ell-1}+x_{2\ell}) \leq 0$
para todo $\ell$.
Assim, a subsequência $\{ s_{2k} \}_{k=1}^\infty$ das somas parciais
é decrescente. Analogamente, $(x_{2\ell}-x_{2\ell+1}) \geq 0$ e, portanto,
\begin{equation*}
s_{2k} = - x_1 + ( x_2 - x_3 ) + \cdots + ( x_{2k-2} - x_{2k-1} ) + x_{2k}
\geq -x_1 .
\end{equation*}
A intuição por trás da estimativa
$0 \geq s_{2k} \geq -x_1$ está ilustrada na \figureref{fig:seralternate}.
\begin{myfigureht}
\myincludegraphics{ser-alternate}{%
Um diagrama no plano mostra os valores alternados menos x com índice n e x com índice n para n de 1 a 8.
Em linhas contínuas que partem do eixo horizontal, vemos o "gráfico" de
menos x de índice 1, x de índice 2, menos x de índice 3, x de índice 4 e assim por diante.
Cada linha contínua sucessiva é mais curta, pois cada x com índice n é menor que o anterior.
Linhas pontilhadas também mostram menos x de índice 2, menos x de índice 4 etc.
As diferenças menos x de índice 1 mais x de índice 2, menos x de índice 3 mais x de índice 4,
menos x de índice 5 mais x de índice 6 e menos x de índice 7 mais x de índice 8 são indicadas
e correspondem às diferenças entre os pares de segmentos consecutivos.
Observamos que essas diferenças ocupam apenas parte do comprimento total
do segmento correspondente a menos x de índice 1.}
\caption{Ilustração de $0 \geq s_{2k} \geq -x_1$ para $k=4$ em uma série alternada.\label{fig:seralternate}}
\end{myfigureht}

Como $\{ s_{2k} \}_{k=1}^\infty$ é decrescente e limitada inferiormente, ela converge.
Seja $a \coloneqq \lim_{k\to\infty} s_{2k}$.
Queremos mostrar que $\lim_{m\to\infty} s_m = a$, e não apenas que isso vale para a subsequência.
Dado $\epsilon > 0$, escolha $M$ de modo que $\sabs{s_{2k}-a} <
\nicefrac{\epsilon}{2}$ sempre que $k \geq M$.
Como $\lim_{n\to\infty} x_n = 0$, podemos aumentar $M$, se necessário,
de modo que
$x_{2k+1} < \nicefrac{\epsilon}{2}$ sempre que $k \geq M$.  
Suponha que $m \geq 2M+1$. Se $m=2k$, então $k \geq M+\nicefrac{1}{2} \geq M$, e
$\sabs{s_{m}-a}=\sabs{s_{2k}-a} < \nicefrac{\epsilon}{2} < \epsilon$.
Se $m=2k+1$, então também temos $k \geq M$. Observe que
...
Observe que existem séries condicionalmente convergentes cujos termos em valor absoluto
tendem a zero tão lentamente quanto se queira.
A série
\begin{equation*}
\sum_{n=1}^\infty \frac{{(-1)}^n}{n^p}
\end{equation*}
converge para valores de $p > 0$ arbitrariamente pequenos, mas não converge
absolutamente quando $p \leq 1$.

\subsection{Reordenações}

Séries absolutamente convergentes se comportam como esperamos. Por exemplo,
podem ser somadas em qualquer ordem. Nada disso vale para séries
condicionalmente convergentes (veja o \exampleref{example:harmonsumanything}
e o \exerciseref{exercise:seriesconvergestoanything}).

Considere uma série
\begin{equation*}
\sum_{n=1}^\infty x_n .
\end{equation*}
Dada uma função bijetiva $\sigma \colon \N \to \N$, a reordenação
correspondente\index{reordenação de uma série} é a série:
\begin{equation*}
\sum_{k=1}^\infty x_{\sigma(k)} .
\end{equation*}
Simplesmente somamos a série em outra ordem.

\begin{prop}
Seja $\sum_{n=1}^\infty x_n$ uma série absolutamente convergente para um número
$x$. Seja $\sigma \colon \N \to \N$ uma bijeção. Então
$\sum_{n=1}^\infty x_{\sigma(n)}$ é absolutamente convergente e converge para $x$.
\end{prop}

Em outras palavras, uma reordenação de uma série absolutamente convergente
converge (absolutamente) para o mesmo número.

\begin{proof}
Seja dado $\epsilon > 0$. Como $\sum_{n=1}^\infty x_n$ é absolutamente convergente,
escolha $M$ de modo que
\begin{equation*}
\abs{\left(\sum_{n=1}^M x_n \right) - x} < \frac{\epsilon}{2}
\qquad \text{e} \qquad
\sum_{n=M+1}^\infty \sabs{x_n} < \frac{\epsilon}{2} .
\end{equation*}
Como $\sigma$ é uma bijeção, existe um número $K$ tal que, para cada
$n \leq M$, existe $k \leq K$ com $\sigma(k) = n$.
Em outras palavras,
$\{ 1,2,\ldots,M \} \subset \sigma\bigl(\{ 1,2,\ldots,K \} \bigr)$.

Para $N \geq K$, seja $Q \coloneqq \max \sigma\bigl(\{ 1,2,\ldots,N \}\bigr)$.
Calculemos
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
Logo, $\sum_{n=1}^\infty x_{\sigma(n)}$ converge para $x$. Para ver que a convergência
é absoluta, aplicamos o argumento acima a $\sum_{n=1}^\infty \sabs{x_n}$ e mostramos
que $\sum_{n=1}^\infty \sabs{x_{\sigma(n)}}$ converge.
\end{proof}

\begin{example} \label{example:harmonsumanything}
Vamos mostrar que a série harmônica alternada
$\sum_{n=1}^\infty \frac{{(-1)}^{n+1}}{n}$, que não converge absolutamente, pode ser
reordenada para convergir para qualquer número.
Os termos de índice ímpar e os de índice par divergem para
mais infinito e menos infinito, respectivamente (prove isso!):
\begin{equation*}
\sum_{m=1}^\infty \frac{1}{2m-1} = \infty, \qquad \text{e} \qquad
\sum_{m=1}^\infty \frac{-1}{2m} = -\infty .
\end{equation*}
Para simplificar, seja $a_n \coloneqq \frac{{(-1)}^{n+1}}{n}$,
seja dado um número arbitrário $L \in \R$ e defina $\sigma(1) \coloneqq 1$.
Suponha que já definimos $\sigma(n)$ para todo $n \leq N$. Se
\begin{equation*}
\sum_{n=1}^N a_{\sigma(n)} \leq L ,
\end{equation*}
então seja $\sigma(N+1) \coloneqq k$, em que $k \in \N$ é o menor ímpar
que ainda não usamos, isto é, $\sigma(n) \neq k$ para todo $n \leq N$.
Caso contrário, seja $\sigma(N+1) \coloneqq k$, em que $k$ é o menor par
que ainda não usamos.

Pela construção, $\sigma \colon \N \to \N$ é injetiva.
Também é sobrejetiva, pois, se continuarmos somando termos ímpares (respectivamente,
pares), eventualmente ultrapassaremos $L$ e passaremos aos termos pares
(respectivamente, ímpares). Assim, alternamos entre eles infinitas vezes.

Por fim, seja $N$ aquele índice $N$ em que acabamos de ultrapassar $L$ e alternar.
Suponha que acabamos de passar dos termos ímpares para os pares e começamos
a subtrair.
Suponha que continuemos subtraindo até algum
$N' > N$, de modo que no passo seguinte voltemos dos pares para os ímpares.
Então
\begin{equation*}
L + \frac{1}{\sigma(N)} \geq \sum_{n=1}^{N} a_{\sigma(n)}
> \sum_{n=1}^{N'} a_{\sigma(n)} > L- \frac{1}{\sigma(N')}.
\end{equation*}
O mesmo vale ao alternarmos na outra direção.
Portanto, a soma até $N'$ está a uma distância inferior a
$\frac{1}{\min \{ \sigma(N), \sigma(N') \}}$ de $L$. Como
alternamos infinitas vezes, $\sigma(N) \to \infty$
e $\sigma(N') \to \infty$. Logo,
\begin{equation*}
\sum_{n=1}^\infty a_{\sigma(n)} = 
\sum_{n=1}^\infty \frac{{(-1)}^{\sigma(n)+1}}{\sigma(n)} = L .
\end{equation*}

Eis um exemplo que ilustra a demonstração. Para $L=1.2$, a ordem
é
\begin{equation*}
1+\nicefrac{1}{3}-\nicefrac{1}{2}+\nicefrac{1}{5}+\nicefrac{1}{7}+\nicefrac{1}{9}-\nicefrac{1}{4}+\nicefrac{1}{11}+\nicefrac{1}{13}-\nicefrac{1}{6}
+\nicefrac{1}{15}+\nicefrac{1}{17}+\nicefrac{1}{19} - \nicefrac{1}{8} + \cdots .
\end{equation*}
Nesse ponto, estamos a uma distância de no máximo $\nicefrac{1}{8}$ do limite.
Veja a \figureref{fig:serrearrange}.
\begin{myfigureht}
\myincludegraphics{ser-rearrange}{%
Um diagrama mostra uma sequência representada por pontos e uma linha horizontal tracejada.
A sequência começa abaixo da linha e, no passo seguinte, fica acima dela.
Depois volta para baixo da linha, leva alguns passos para ficar acima dela
e, no passo seguinte, fica novamente abaixo.
E assim por diante, com passos cada vez menores.}
\caption{As primeiras 14 somas parciais da reordenação que converge
para $1.2$.\label{fig:serrearrange}}
\end{myfigureht}
\end{example}

\subsection{Multiplicação de séries}

Como já mencionamos,
multiplicar séries é um pouco mais difícil do que somá-las.
Se pelo menos uma das séries converge
absolutamente, podemos usar o teorema a seguir. Para esse resultado, é
conveniente iniciar as séries em 0, e não em 1.

\begin{thm}[\myindex{teorema de Mertens}\footnote{Demonstrado pelo matemático alemão
\href{https://en.wikipedia.org/wiki/Franz_Mertens}{Franz Mertens}
(1840--1927).}]
Suponha que $\sum_{n=0}^\infty a_n$ e $\sum_{n=0}^\infty b_n$ sejam duas séries convergentes, com limites
respectivos $A$ e $B$. Suponha que pelo menos uma das séries
converge absolutamente. Defina
\begin{equation*}
c_n \coloneqq a_0 b_n + a_1 b_{n-1} + \cdots + a_n b_0 = \sum_{i=0}^n a_i b_{n-i} .
\end{equation*}
Então a série $\sum_{n=0}^\infty c_n$
converge para $AB$.
\end{thm}

A série $\sum_{n=0}^\infty c_n$ é chamada de \emph{\myindex{produto de Cauchy}} de
$\sum_{n=0}^\infty a_n$ e $\sum_{n=0}^\infty b_n$.

\begin{proof}
Suponha que $\sum_{n=0}^\infty a_n$ converge absolutamente e seja dado
$\epsilon > 0$.
Nesta demonstração, em vez de escolher estimativas complicadas apenas para que
a estimativa final seja menor que $\epsilon$,
vamos obter uma estimativa que dependa de $\epsilon$
e possa ser tornada arbitrariamente pequena.

Escreva
\begin{equation*}
A_m \coloneqq \sum_{n=0}^m a_n , \qquad B_m \coloneqq \sum_{n=0}^m b_n .
\end{equation*}
Reescrevemos a $m$-ésima soma parcial de $\sum_{n=0}^\infty c_n$:
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
Certamente podemos tornar pequeno o segundo termo do lado direito.
O ponto é controlar o primeiro termo.
Escolha $K$ de modo que, para todo $m \geq K$, tenhamos
$\sabs{A_m - A} < \epsilon$ e também
$\sabs{B_m - B} < \epsilon$.
Como $\sum_{n=0}^\infty a_n$ converge absolutamente,
existe um $K$ suficientemente grande tal que, para todo $m \geq K$,
\begin{equation*}
\sum_{n=K}^m \sabs{a_n} < \epsilon .
\end{equation*}
Como $\sum_{n=0}^\infty b_n$ converge,
$B_{\text{max}} \coloneqq \sup \bigl\{ \sabs{ B_n - B } : n = 0,1,2,\ldots
\bigr\}$
é finito. Seja $m \geq 2K$.
Em particular, $m-K+1 > K$. Portanto,
\begin{equation*}
\begin{split}
\sum_{n=0}^m
  \sabs{ B_n -  B } \sabs{a_{m-n}}
...
\end{split}
\end{equation*}
A expressão entre parênteses no lado direito
é um número fixo.
Assim, podemos tornar o lado direito arbitrariamente pequeno escolhendo
$\epsilon> 0$ suficientemente pequeno. Logo, $\sum_{n=0}^\infty c_n$ converge para $AB$.
\end{proof}

\begin{example}
Se ambas as séries forem apenas condicionalmente convergentes, a série produto de Cauchy
pode nem sequer convergir.
Considere $a_n = b_n = {(-1)}^n \frac{1}{\sqrt{n+1}}$.
A série $\sum_{n=0}^\infty a_n = \sum_{n=0}^\infty b_n$
converge pelo critério das séries alternadas; no entanto, não converge
absolutamente, como se pode verificar pelo critério da série $p$. Vejamos
o produto de Cauchy.
\begin{equation*}
...
= 1 .
\end{equation*}
Os termos não tendem a zero e, portanto, $\sum_{n=0}^\infty c_n$ não pode convergir.
\end{example}

\subsection{Séries de potências}

Fixe $x_0 \in \R$.
Uma \emph{\myindex{série de potências}} em torno de $x_0$
é uma série da forma
\begin{equation*}
\sum_{n=0}^\infty a_n {(x-x_0)}^n .
\end{equation*}
Uma série de potências é, na verdade, uma função de $x$, e
muitas funções importantes em análise podem ser escritas
como séries de potências. Adotamos a convenção
$0^0 = 1$ (isto é, quando $x=x_0$ e $n=0$).

Dizemos que uma série de potências é
\emph{convergente}\index{convergente!série de potências} se
existe pelo menos um $x \neq x_0$ para o qual a série converge.
Se $x=x_0$, a série sempre converge, pois todos os termos, exceto o primeiro, são zero.
Se a série não converge em nenhum ponto $x \neq x_0$, dizemos que
ela é \emph{divergente}\index{divergente!série de potências}.
...
\end{equation*}
converge absolutamente para todo $x \in \R$, pelo critério da razão.
...
0.
\end{equation*}
Lembre-se, do cálculo, que essa série converge para $e^x$.
\end{example}
...
\end{equation*}
converge absolutamente para todo $x \in (-1,1)$, pelo critério da razão:
\begin{equation*}
\lim_{n \to \infty}
\abs{
\frac{\bigl(1/(n+1) \bigr) \, x^{n+1}}{(1/n) \, x^{n}}
...
\sabs{x} < 1 .
\end{equation*}
A série converge em $x=-1$, pois
$\sum_{n=1}^\infty \frac{{(-1)}^n}{n}$ converge
pelo critério das séries alternadas.
Mas a série de potências não converge absolutamente em $x=-1$, pois
$\sum_{n=1}^\infty \frac{1}{n}$ diverge.
A série diverge em $x=1$.
Se $\sabs{x} > 1$, a série diverge pelo critério da razão.
\end{example}
...
\end{equation*}
diverge para todo $x \neq 0$. Vamos aplicar o critério da raiz
\begin{equation*}
\limsup_{n\to\infty}
\,
\sabs{n^n x^n}^{1/n}
...
Em geral, a convergência das séries de potências funciona de modo análogo
aos três exemplos acima.

\begin{prop} \label{prop:powerserrealradius}
Seja $\sum_{n=0}^\infty a_n {(x-x_0)}^n$ uma série de potências.
Se a série for convergente, então ou converge absolutamente para
todo $x \in \R$, ou
existe um número $\rho$ tal que
a série converge absolutamente no intervalo
$(x_0-\rho,x_0+\rho)$ e diverge quando $x < x_0-\rho$ ou $x > x_0+\rho$.
\end{prop}

O número $\rho$ é chamado de \emph{\myindex{raio de convergência}} da
série de potências. Escrevemos $\rho = \infty$ se a série converge para
todo $x$, e $\rho = 0$ se a série é divergente.
Nos extremos, isto é, se $x = x_0+\rho$ ou $x = x_0-\rho$,
a proposição não afirma nada,
e a série pode ou não convergir.
Veja a \figureref{ps:convfig}.
No \exampleref{ps:1kex},
o raio de convergência é $\rho=1$;
no \exampleref{ps:expex}, o raio de convergência é $\rho=\infty$;
e no \exampleref{ps:divergeex}, o raio de convergência é $\rho=0$.

\begin{myfigureht}
\myincludepdft{ps-conv}{%
Um diagrama da reta real com três pontos marcados, da esquerda para a
direita: x₀ − ρ, x₀ e x₀ + ρ.
A parte da reta à esquerda de x₀ − ρ e a parte à direita de x₀ + ρ
estão identificadas como "diverge". O intervalo entre x₀ − ρ
e x₀ + ρ está sombreado e identificado como "converge absolutamente".
Os pontos x₀ − ρ e x₀ + ρ estão marcados com pontos de interrogação.}
\caption{Convergência de uma série de potências.\label{ps:convfig}}
\end{myfigureht}
...
\sabs{x-x_0} R .
\end{equation*}
Se $R = \infty$, então $L=\infty$ para todo $x \neq x_0$, e
a série diverge pelo critério da raiz.
Por outro lado,
se $R = 0$, então $L=0$ para todo $x$,
e a série converge absolutamente para todo $x$.

Suponha que $0 < R < \infty$.
...
\sabs{x-x_0} > \nicefrac{1}{R} .
\end{equation*}
Tomando $\rho \coloneqq \nicefrac{1}{R}$, concluímos a demonstração.
\end{proof}

Pode ser útil enunciar como uma proposição separada
o que aprendemos na demonstração.

\begin{prop}
Seja $\sum_{n=0}^\infty a_n {(x-x_0)}^n$ uma série de potências, e seja
\begin{equation*}
R \coloneqq \limsup_{n\to\infty} {\sabs{a_n}}^{1/n} .
\end{equation*}
Se $R = \infty$, a série de potências é divergente. Se
$R=0$, então ela converge em toda parte. Caso contrário,
o raio de convergência é $\rho = \nicefrac{1}{R}$.
\end{prop}

Muitas vezes, escreve-se o raio de convergência como $\rho = \nicefrac{1}{R}$ nos
três casos, entendendo-se qual deve ser o valor de $\rho$ quando $R = 0$ ou $R =
\infty$.

\pagebreak[2]
Séries de potências convergentes podem ser somadas e multiplicadas entre si,
além de multiplicadas por constantes.
A proposição tem uma demonstração direta, baseada no que sabemos sobre séries
em geral e séries de potências em particular. Deixamos a demonstração a cargo do leitor.

\begin{prop}
Sejam $\sum_{n=0}^\infty a_n {(x-x_0)}^n$ e
$\sum_{n=0}^\infty b_n {(x-x_0)}^n$ duas séries de potências convergentes
com raio de convergência pelo menos $\rho > 0$, e seja $\alpha \in \R$. Então,
para todo $x$ tal que $\sabs{x-x_0} < \rho$, temos 
\begin{equation*}
...
Para todo $x$ com $\sabs{x-x_0} < \rho$, temos duas séries convergentes; portanto,
a soma termo a termo e a multiplicação por constantes
seguem da seção anterior.
Como, para esses valores de $x$, as séries convergem absolutamente,
podemos aplicar o teorema de Mertens para calcular o produto das duas séries.
Consequentemente, depois de efetuar as operações algébricas, o
raio de convergência da série resultante é pelo menos $\rho$.
O raio de convergência do resultado pode ser estritamente maior do que
o raio de convergência de qualquer uma das séries iniciais.
Veja os exercícios.

Vejamos alguns exemplos de séries de potências.
Polinômios são simplesmente séries de potências finitas: um polinômio
é uma série de potências em que
os $a_n$ são zero para todo $n$ suficientemente grande. Desenvolvemos
um polinômio como uma série de potências em torno de qualquer ponto $x_0$,
escrevendo-o como um polinômio em $(x-x_0)$. Por exemplo,
$2x^2-3x+4$, escrito como série de potências em torno de $x_0 = 1$, é
\begin{equation*}
2x^2-3x+4 = 3 + (x-1) + 2{(x-1)}^2 .
\end{equation*}

Também podemos desenvolver
\emph{\myindex{funções racionais}} (isto é, quocientes de polinômios)
em séries de potências, embora não demonstremos esse fato por completo aqui.
Observe que uma série para uma função racional define essa função apenas
em um intervalo, mesmo que ela esteja definida em outros pontos. Por exemplo, para
a \emph{\myindex{série geométrica}}, temos, quando
$x \in (-1,1)$,
\begin{equation*}
\frac{1}{1-x} =
\sum_{n=0}^\infty x^n .
\end{equation*}
A série diverge quando $\sabs{x} > 1$, embora $\frac{1}{1-x}$ esteja
definida para todo $x \neq 1$.

Podemos usar a série geométrica, junto com as regras de adição e
multiplicação de séries de potências, para desenvolver funções racionais como séries de
potências em torno de $x_0$,
desde que o denominador não seja zero em $x_0$. Enunciamos sem
demonstração que isso sempre é possível e apresentamos um exemplo desse
cálculo usando a série geométrica.

\begin{example}
Vamos desenvolver $\frac{x}{1+2x+x^2}$ em série de potências em torno da origem ($x_0 = 0$) e
encontrar o raio de convergência.

Escreva $1+2x+x^2 = {(1+x)}^2 = {\bigl(1-(-x)\bigr)}^2$ e suponha
que $\sabs{x} < 1$. Calculemos
\begin{equation*}
\begin{split}
\frac{x}{1+2x+x^2}
&=
...
\end{split}
\end{equation*}
Usando a fórmula do produto de séries,
obtemos $c_0 = 1$, $c_1 = -1 -1 = -2$, $c_2 = 1+1+1 = 3$ etc.
Portanto, para $\sabs{x} < 1$, 
\begin{equation*}
\frac{x}{1+2x+x^2}
=
\sum_{n=1}^\infty {(-1)}^{n+1} n x^n .
\end{equation*}
O raio de convergência é pelo menos 1. Deixamos ao leitor a tarefa de
verificar que o raio de convergência é exatamente 1.
\end{example}

Você pode usar o método de frações parciais estudado no cálculo.
Por exemplo, para encontrar a série de potências de $\frac{x^3+x}{x^2-1}$ em 0, escreva
\begin{equation*}
\frac{x^3+x}{x^2-1}
=
...
\begin{exercise}
Suponha que tanto $\sum_{n=0}^\infty a_n$ quanto $\sum_{n=0}^\infty b_n$
convirjam absolutamente.
Mostre que a série produto, $\sum_{n=0}^\infty c_n$, em que
$c_n = a_0 b_n + a_1 b_{n-1} + \cdots + a_n b_0$, também converge absolutamente.
\end{exercise}

\begin{exercise}[Desafiador] \label{exercise:seriesconvergestoanything}
Seja $\sum_{n=1}^\infty a_n$ uma série condicionalmente convergente.
Mostre que, dado um $x \in \R$ arbitrário,
existe uma reordenação de $\sum_{n=1}^\infty a_n$
tal que a série reordenada converge para $x$.
Dica: veja o \exampleref{example:harmonsumanything}.
\end{exercise}
...
\begin{enumerate}[a)]
\item
Mostre que a série harmônica alternada $\sum_{n=1}^\infty \frac{{(-1)}^{n+1}}{n}$
tem uma reordenação
tal que, para todo intervalo $(x,y)$, existe uma soma parcial $s_n$
da série reordenada com $s_n \in (x,y)$.
\item
Mostre que a reordenação que você encontrou não converge.
Veja o \exampleref{example:harmonsumanything}.
\item
Mostre que, para todo $x \in \R$, existe uma subsequência das
somas parciais $\{ s_{n_k} \}_{k=1}^\infty$ da sua reordenação tal que
$\lim\limits_{k\to\infty} s_{n_k} = x$.
\end{enumerate}
\end{exercise}

\begin{exercise}
Para cada uma das séries de potências a seguir, determine se ela converge
e, em caso afirmativo, encontre seu raio de convergência.
No item f), observe o fatorial no expoente.
...
\begin{exercise}
Suponha que $\sum_{n=0}^\infty a_n x^n$ converge para $x=1$.
\begin{enumerate}[a)]
\item
O que se pode afirmar sobre o raio de convergência?
\item
Se, além disso, você sabe que em $x=1$ a convergência não é absoluta,
o que pode concluir?
\end{enumerate}
\end{exercise}
...
\begin{exercise}
Desenvolva
$\dfrac{x}{4-x^2}$ em série de potências em torno de $x_0 = 0$
e calcule seu raio de convergência.
\end{exercise}
...
\begin{enumerate}[a)]
\item
Encontre um exemplo em que os raios de convergência de $\sum_{n=0}^\infty a_n x^n$ e
$\sum_{n=0}^\infty b_n x^n$ sejam ambos iguais a 1, mas o raio de convergência
da soma das duas séries seja infinito.
\item
(Mais difícil)
Encontre um exemplo em que os raios de convergência de $\sum_{n=0}^\infty a_n x^n$ e
$\sum_{n=0}^\infty b_n x^n$ sejam ambos iguais a 1, mas o raio de convergência
do produto das duas séries seja infinito.
\end{enumerate}
\end{exercise}

\begin{exercise}
Descubra como calcular o raio de convergência usando o critério da razão.
Isto é, suponha que $\sum_{n=0}^\infty a_n x^n$ seja uma série de potências e que
$R \coloneqq \lim_{n\to\infty} \frac{\sabs{a_{n+1}}}{\sabs{a_n}}$ exista ou seja $\infty$.
Encontre o raio de convergência e prove sua afirmação.
\end{exercise}
...
\begin{enumerate}[a)]
\item
Prove que $\lim_{n\to\infty} n^{1/n} = 1$ usando o seguinte procedimento: escreva $n^{1/n} = 1+b_n$ e
observe que $b_n > 0$ quando $n > 1$. Em seguida, mostre que ${(1+b_n)}^n \geq
\frac{n(n-1)}{2}b_n^2$ e use isso para provar que $\lim\limits_{n\to\infty} b_n = 0$.
\item
Use o resultado do item a) para mostrar que,
se $\sum_{n=0}^\infty a_n x^n$ é uma série de potências convergente com raio de convergência $R$,
então $\sum_{n=0}^\infty n a_n x^n$ também converge com o mesmo raio de convergência.
\end{enumerate}
\end{exercise}
\end{samepage}

\begin{exnote}
Há diferentes noções de somabilidade (convergência)
de uma série,
além daquela que vimos.
Uma delas, bastante comum, é a \emph{\myindex{somabilidade de Ces\`aro}}%
\footnote{Nomeada em homenagem ao matemático italiano
\href{https://en.wikipedia.org/wiki/Ernesto_Ces\%C3\%A0ro}{Ernesto Ces\`aro}
(1859--1906).}. Seja $\sum_{n=1}^\infty a_n$ uma série
e seja $s_n$ sua $n$-ésima soma parcial. Dizemos que a série é
somável no sentido de Ces\`aro para $a$ se
\begin{equation*}
...
\begin{enumerate}[a)]
\item
Se $\sum_{n=1}^\infty a_n$ converge para $a$ (no sentido usual), mostre que
$\sum_{n=1}^\infty a_n$ é somável no sentido de Ces\`aro (veja acima) para $a$.
\item
Mostre que, no sentido de Ces\`aro, $\sum_{n=1}^\infty {(-1)}^n$ é somável para
$\nicefrac{-1}{2}$.
\item
Seja $a_n \coloneqq k$ quando $n = k^3$ para algum $k \in \N$,
$a_n \coloneqq -k$ quando $n = k^3+1$ para algum $k \in \N$;
caso contrário,
seja $a_n \coloneqq 0$. Mostre que $\sum_{n=1}^\infty a_n$ diverge no sentido usual
(de fato, tanto a sequência de termos quanto as somas parciais são ilimitadas), mas é
somável no sentido de Ces\`aro para 0 (o que pode parecer um pouco paradoxal à primeira vista).
\end{enumerate}
\end{exercise}

\begin{exercise}[Desafiador]
Mostre que a monotonicidade no critério das séries alternadas
é necessária. Isto é, encontre uma sequência de números reais positivos
$\{ x_n \}_{n=1}^\infty$ com $\lim_{n\to\infty} x_n = 0$, mas tal que
$\sum_{n=1}^\infty {(-1)}^n x_n$ diverge.
\end{exercise}

\begin{exercise}
Encontre uma série
$\sum_{n=1}^\infty x_n$ que converge,
mas $\sum_{n=1}^\infty x_n^2$ diverge.
Dica: compare com o \exerciseref{exercise:squareseriesconv}.
\end{exercise}

\begin{exercise}
Suponha que $\{ c_n \}_{n=1}^\infty$ seja uma sequência. Prove que, para todo $r \in (0,1)$,
existe uma sequência estritamente crescente $\{ n_k \}_{k=1}^\infty$ de números naturais
($n_{k+1} > n_k$) tal que
\begin{equation*}
\sum_{k=1}^\infty c_k x^{n_k}
\end{equation*}
converge absolutamente para todo $x \in [-r,r]$.
\end{exercise}

\begin{exercise}[Tonelli/Fubini para somas, desafiador]
\label{exercise:tonellifubiniforsums}%
\index{Fubini para somas}\index{Tonelli para somas}%
Seja $\{ x_{k,\ell} \}_{k=1,\ell=1}^\infty$ uma sequência com dois
índices e seja $\sigma \colon \N \to \N^2$ uma bijeção.
Considere as séries
\begin{equation*}
...
\end{equation*}
As expressões ii) e iii) são séries de séries; dizemos que convergem
se a série interna sempre converge e, em seguida, a série externa também
converge.
\begin{enumerate}[a)]
\item
(Tonelli) Suponha que $x_{k,\ell} \geq 0$ para todos $k,\ell$.
Mostre que as três séries i), ii), iii) ou divergem (para $\infty$)
ou convergem todas para o mesmo número.
No caso de divergência, algumas das séries \myquote{internas} podem ser iguais a infinito;
nesse caso, consideramos que a soma inteira diverge.
\item
(Fubini) Suponha que i) converge absolutamente. Mostre que ii) e iii)
convergem e que ambas convergem para o mesmo número que i).
\end{enumerate}
\end{exercise}
