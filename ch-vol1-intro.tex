...
\begin{myfigureht}
\myincludepdft{sets}{%
A diagram of example sets S and T.  The set S includes 0, 1, and 2,
and the set T includes 0 and 2.  There is an element 7 not
...
and $A \neq B$.  We write $A \subsetneq B$.\glsadd{not:subsetneq}
\end{enumerate}
\end{defn}

For the example $S$ and $T$ defined above, $T \subset S$, but
$T \neq S$.  So $T$ is a proper subset of $S$.
...
Venn diagrams of set operations: First a diagram of A union B,
with the inside of A or B shaded.
Then a diagram of A intersect B where only the intersection
A and B is shaded.  Then a diagram of A set minus B, where
everything in A but not in B is shaded.  Finally a diagram
of B complement, where everything not in B is shaded.}
...
We called the result above a \emph{Theorem}, while most results we call
...
possess the \emph{\myindex{well ordering property}}.
...
\end{split}
\end{equation*}
By the principle of induction, the equation holds for all
$n \in \N \cup \{ 0 \}$.
\end{example}
...
\emph{\myindex{graph}} of the function.
...
Two sets of dots are labeled 1, 2, 3, 4 and a, b, c, d.
There is an arrow from 1 to b, from 2 to d, from 3 to c,
and from 4 to b.  A list of example images is on the right:
f applied to the set containing 1,2,3,4 equals the set containing b, c, d.
f applied to the set containing 1,2,4 equals the set containing b, d.
f applied to the set containing 1 equals the set containing b.
f inverse applied to the set containing a,b,c equals the set containing 1,3,4.
f inverse applied to the set containing a equals the empty set.
f inverse applied to the set containing b equals the set containing 1,4.}
