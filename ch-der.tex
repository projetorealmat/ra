...
f(y)-f(x) = f'(c)(y-x) .
\end{equation*}
As $f'(c) \geq 0$ and $y-x > 0$, we have $f(y) - f(x) \geq 0$ or $f(x) \leq
f(y)$, and so
$f$ is increasing.
...
The proof follows by letting $g$ be the difference of $f$
and a linear function with derivative $y$.
The new function $g$ reduces the problem
...
A graph of a function with three points, a, c, b, in this order,
marked on the horizontal axis.  Tangent lines are drawn at
the corresponding points on the graph.  From left to right,
the first line slopes up and is labeled g prime of a is greater than 0.
The second line is horizontal and is labeled g prime of c equals 0.
The third line slopes down and is labeled g prime of b is less than 0.}
...
On the left is a graph of a function that oscillates between
the x-axis and the function x squared, whose graph is given
as a dashed line.
On the right is a function that oscillates faster and faster
as it gets close to the origin, looking somewhat like the graph
of sine of 1 over x, however the entire graph appears sheared a bit
so that it goes upwards as well.}
...
\sabs{x} .
\end{equation*}
And, of course, as $x$ tends to zero, $\sabs{x}$ tends to zero, and hence
...
write $C^1(I)$ for the set of continuously differentiable functions defined
on $I$.
...
$\babs{f(x)-f(y)} \leq \sabs{x-y}^2$ for all $x$ and $y$.  Show that
...
such that
$f'(x) \neq 0$ for all $x \in (a,b)$.  Suppose there
exists
...
Remark: It is important that $(a,b)$ is a bounded interval.
\end{exercise}
...
line and is therefore tangent to the vertical y-axis at the origin.}
